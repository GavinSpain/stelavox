# Stelavox Director Architecture v2.10

**Status:** Design + V1.x-B.2 implementation absorption + V1.x-C plan-model + per-org-BYOK + V1.x-D UI substrate + V1.x-E admin dashboard + V1.x-F failure-mode UX + 2026-05-21 amendment-surface simplification + 2026-06-11 Phase 9.2 cloud-transport cutover + 2026-06-11 Phase 9.D context-node create+link teaching — supersedes parts of `stelavox_technical_architecture_v2_3.md` §8 (now v2.20).

> **Phase 9.D — Director context-node create + link (2026-06-11; DR-051 SHIPPED).** §4 tool registry V1.29 (17 tools = v1.28's 17; pure prompt revision, no tool change). §6 generate_context semantics: the Director may target either an existing context node OR a structural node — when targeting structural, the executor at `lib/director/workflow-executor.ts:496-578` auto-creates a context node of the requested `context_type`, inserts a `node_context_links` row of type `structural_to_context` from the structural target to the new node, and re-targets the workflow_step at the new node before dispatch. The auto-create-and-link substrate has been live since Mars-series 2026-05-08 (SU-J11-2 Option B); v1.29 brings the prompt into parity so the model plans `generate_context` against structural targets directly instead of avoiding the path. Step shapes line additionally documents the optional `seed_content` parameter (which seeds the auto-created node's name + initial summary). **Back-documented in this revision:** v1.25 (M-186 — drop the expand `instruction` parameter drift), v1.26 (M-187 — align step shapes), v1.27 (M-208 — completeness model), v1.28 (M-209 — inferential discipline) — all four had shipped without entries here. H-08 propose-only invariant preserved: the executor's auto-create runs at dispatch time (post-author-approval), not inside the Director's tool-use loop. No new tools; tool registry V1.29 unchanged. No Inviolable changes; verdigris-use count remains 12.


> **Phase 9.2 cloud-dispatch transport — Option E (pg_net + pg_cron) is now canonical (2026-06-11).** The original V2 scheduler model assumed a long-lived TS LISTEN/NOTIFY listener as the dispatch path. That model still runs on `npm run dev` (sub-second latency, kept for fast local iteration) but is unsuitable for Vercel (no always-on process; spawning a paid worker for a sub-2s tick is wasteful). **The cloud transport is now Postgres → HTTPS, double-gated, fire-and-forget.** M-216 + M-217 add `invoke_scheduler_endpoint(p_path TEXT)` SECURITY DEFINER which (a) reads `scheduler.cloud_dispatch_base_url` from `platform_config` and skips if unset, (b) reads `cron_secret` from Vault and skips if unset, then (c) calls `net.http_post(url, Bearer headers, body, timeout_milliseconds := 30000)` inside a BEGIN/EXCEPTION block whose every failure path is `RAISE WARNING` — pg_net itself is async, so the trigger transaction never blocks. **Three integration points:** a new AFTER-INSERT trigger on `agent_jobs` posts to `/api/cron/dispatcher-tick` whenever a queued row lands; the existing `agent_jobs_notify_completion` trigger now also posts to dispatcher-tick on terminal-state transitions; new pg_cron jobs invoke `/api/cron/dispatcher-tick` every minute (`dispatcher_sweep_http` safety floor), `/api/cron/route-sample` every minute (metrics), `/api/cron/poll-batches` every 5 minutes, and the synthetic probes daily — all via the same helper. **Migration count moves 214 → 217.** Local LISTEN/NOTIFY listener untouched. Both transports are dispatch-only signals; CAS-on-claim makes double-fire safe. **Activated against stelavox-dev**: `scheduler.cloud_dispatch_base_url = https://stelavox.vercel.app`; `net._http_response` records 200 OK every minute autonomously. DR-063 + DR-067 + DR-117 closed. See `stelavox_phase9_1_2_test_report_v1_0.md` for activation evidence. **§15 V1.x backlog "scheduler-listener cloud home"** is closed by this transport; the listener-on-always-on-worker design path no longer applies. No new tools; tool registry V1.24 unchanged. No Inviolable changes; verdigris-use count remains 12.


> **Phase 9 deliverables-register supersession (2026-06-10):** §16.3 (V2 backlog) and §16.4 (calibration questions) are superseded as standalone inventories by the **V1 Deliverables Register** (`stelavox_v1_deliverables_register_v1_0.md`) — every item now carries a DR number and an author-locked bucket. §16.3 items: all V2+ except extended-thinking UX surfacing (DR-008 → V1.x). §16.4 calibration items: all V1.x, with a NEW V1 requirement DR-121 — the admin dashboard must surface every calibration-relevant metric from Day 1 so post-launch tuning has data. Director-relevant V1 pull-forwards from the review: **DR-051 — Director must be able to create a context node AND link it to a content node** (resolves the Phase 5d SU-J11-2 create-and-fill semantics gap; needs a Tier-B design pass against the tool registry; H-08 propose-only applies); **DR-020 — FailureToast/FailureBanner per-surface adoption**; **DR-066 — "Director was interrupted — resume?" banner**. Sections §16.3/§16.4 retained below for historical traceability; the register wins on any disagreement.

> **Director simplification — amendment surface removed (2026-05-21):** §4 tool registry V1.24 (17 tools = v1.23's 17 with `propose_brief_amendment` swapped for `propose_workflow`; net count unchanged). §6 Brief model: each `brief_stages` row now carries EITHER a workflow (concrete steps planned at brief-proposal time) OR a prompt (system invokes the Director with the prompt when the stage's trigger fires; the Director responds via `propose_workflow`; the iteration runner attaches the workflow + auto-approves+dispatches when `brief.auto_approve_workflow_proposals` is true). The five-amendment-type surface (goal_text / preferences / add_stage / modify_pending_stage / remove_pending_stage), the `brief_amendments` table, the `apply_brief_amendment` RPC, the `BriefAmendmentCard` UI, the `/api/brief/amendments/*` routes, and the `target_path` UUID-vs-order-vs-dotted-JSONPath drift are all gone. Stage status enum: `'proposing'` renamed to `'planning'` (terminology aligns with the new system-driven model). Stage trigger types narrowed to `('after_stage','manual')` — `'scheduled_at'` and `'compound'` dropped (never exercised in V1.x). One active brief per document (M-126's multi-active partial-unique-index drop is reversed by M-183). **§16 backlog updates:** Brief amendments removed from V2 candidates — the model proved unreliable in practice; if a real mid-flight-edit scenario emerges post-V1 launch, it lands as a fresh focused tool, not a rebuild. **No Inviolable changes** — the simplification removes UI surface area (BriefAmendmentCard), doesn't add any.

> **V1.x-F close-out absorption (2026-05-19):** §4 tool registry V1.10 (19 tools = v1.9's 18 + `report_capability_limit` synthetic write-tool; propose-only per H-08). §10 failure-class UX implementation-complete at substrate level: per-class user-facing message templates in platform_config (6 keys; M-147) + FailureToast Class A (--color-info; surfaces on 2nd retry onward per D1) + Class C (--color-status-review; surfaces only when back-off >= class_c_min_pause_seconds=15 per D2) + FailureBanner Class D (--color-error left border with remediation block) + Class E (full --color-error border with mailto: support CTA per D3). Class B (Resume) already shipped V1.x-D; per-surface adoption of the new components happens incrementally. §17 admin probes section updated: `workflow_expand` + `refine_accept` now have real implementations (lib/admin/probes/workflow-expand.ts + refine-accept.ts) against probe-fixture data seeded by scripts/seed-probe-fixtures.ts; pg_cron auto-run staggered daily 04:15/30/45 UTC (M-148) + Vercel-Cron fallback POST /api/cron/run-probes; the V1.x-E `pendingImplementation` stub deleted. **§16 backlog updates:** `refine_accept` accept-path exercise promotes from V1.x-F polish to V2 (cleanup needs to roll back the version-bump + node_versions row insert). 30-day rate-limit retention bump (D-F3-1 locked: keep 7-day default from M-145). Per-surface FailureToast/FailureBanner adoption incremental. **No new §16 backlog items.** No Inviolable changes; CapabilityLimitCard + FailureToast + FailureBanner add ZERO verdigris uses — verdigris-use count remains nine.

> **V1.x-E close-out absorption (2026-05-19):** §17 admin dashboard substrate implementation-complete. The metrics rollup layer shipped in V1.x-B.2.4 (`metrics_minute_buckets` + `dispatcher_tick_samples` + `route_capacity_samples`) now feeds a real /admin page with live counters / queue depth by class / Anthropic ITPM headroom / dispatch-rate sparkline / failures-by-class with auto-recovery rate / spend leaders (top orgs by usage + by-model credits) / capacity alerts (three kinds: ITPM sustained-window crossover / queue-oldest stale / failure-rate threshold) / synthetic probes (manual trigger; `director_small` real Anthropic ping; two stubs for V1.x-F). Anthropic per-call rate-limit headers captured into a new `anthropic_rate_limit_samples` table via custom fetch wired into `AnthropicProvider` constructor — fire-and-forget, errors swallowed; BYOK route bypasses (BYOK call returns user's headers, polluting platform headroom signal). Admin auth: `PLATFORM_ADMIN_EMAILS` env-var allowlist for V1; V2 candidate to migrate to `users.is_platform_admin` column once a real admin user base exists. Capacity alerts evaluated pull-style at /admin page-load — push notifications remain a §16.3 V2 candidate. **§16 backlog updates:** `workflow_expand` + `refine_accept` real probe implementations promote to V1.x-F polish (substrate stubs ship in V1.x-E recording `probe_implementation_pending_v1xf`); pg_cron auto-run for probes promotes to V1.x-F polish. **No new §16 backlog items.** No Inviolable changes; admin surfaces use `--color-status-review` / `--color-error` / `--color-info` only — verdigris-use count remains nine.
