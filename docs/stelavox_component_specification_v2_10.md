...
> **VersionHistory click-to-preview pane (2026-05-24):** `components/detail/VersionHistory.tsx` adds a click-to-select model to each version row, and renders a persistent **`VersionPreviewPane`** (NEW — `components/detail/VersionPreviewPane.tsx`) below the list that shows the selected version's Summary + Prose + Notes. The preview pane is read-only — no editor mounts, no autosave, no toolbar — so the author can read the historical content before deciding to restore without having to restore first and undo. Discoverability gap closed: prior surface offered only a 320px hover-tooltip word-diff (still present), which never gave a faithful read of long historical prose. Selected-row visual state is a 2px left border in `--color-text-primary` plus `--color-bg-elevated` background — **NOT verdigris** (selection is informational, not affirmative action; Inviolable #2 unchanged, verdigris-use count remains nine). Preview pane honours the typeface boundary (Inviolable #4) via `data-editor="summary|notes|prose"` selectors that pick up the existing globals.css typeface rules: summary + notes render in Inter 13px, prose renders in Lora at `--text-prose-panel` — visually faithful to the live editor. Tiptap JSON is rendered via a small recursive renderer (`renderTiptapJson` exported for unit-test from VersionPreviewPane.tsx; 16 vitest cases at `tests/unit/version-preview-renderer.test.ts`) supporting paragraph, heading 1-6, bulletList, orderedList, listItem, blockquote, horizontalRule, hardBreak; inline marks bold, italic, link, code; unknown nodes/marks fall through (text never lost). Per-row click toggles selection (click again deselects). Restore button click is propagation-stopped so it doesn't also toggle the row's preview. Five new Playwright cases at `tests/ui/version-history.spec.ts` TC-U-24..28 cover empty pane, click-to-render, swap, deselect, and the Restore-doesn't-toggle-selection regression. Per-node cache cleared on `nodeId` change so a tree-navigation cannot leak content from the previous node into the new preview. No API change — `/api/nodes/[id]/versions/[v]` already returns summary + prose + notes + metadata. The Phase 3/Phase 6 split note in §5.11 below is now stale (Phase 6.C shipped Restore in v1.37); TC-U-20's "no Restore button" assertion is a pre-existing test failure outside this change's scope.
...
| **Click selection** (v2.19) | Clicking a row selects it; clicking the same row again deselects. Selected row gets a 2px `--color-text-primary` left border + `--color-bg-elevated` background. **NOT verdigris** — selection is informational, not affirmative action. `data-version-selected="true|false"` exposed for tests. Enter/Space on a focused row also toggles. |
| **Preview pane** (v2.19) | Renders below the list; shows the selected version's Summary + Prose + Notes; read-only (no editor, no autosave, no toolbar); typefaces honour Inviolable #4 (Inter for summary/notes, Lora for prose). See §5.11.1. |
| Load more | "Show [N] more versions…" link below the initial 7 shown |

Restoring creates a new version — does not delete history.

🔒 **Phase 3 vs Phase 6 split** (historical note — superseded by Phase 6.C ship in CLAUDE.md v1.37). The list, current-version star, hover diff tooltip, and "Show N more" pagination shipped in Phase 3. The **Restore button + RestoreConfirmModal + lock-aware disabled states + `restore_node_version` RPC** shipped in Phase 6.C (M-157). The v2.19 amendment adds the click-to-preview pane on top of that surface.

---

### 5.11.1 VersionPreviewPane

**File:** `components/detail/VersionPreviewPane.tsx`

Click-to-preview surface for the History tab. Mounted inside `VersionHistory` and driven by the selected-version state. Reads from the same `/api/nodes/[id]/versions/[v]` endpoint the hover-diff already uses, via a shared per-node cache (cleared on `nodeId` change). Hidden when the version list is empty (the existing empty-state copy covers that case).

| Property | Value |
|---|---|
| Empty state | "Click a version above to preview its contents." Dashed border, Inter 12px `--color-text-muted`, centred. `data-testid="version-preview-empty"`. |
| Loading | "Loading v{N}…" Inter 12px `--color-text-muted`. `data-testid="version-preview-loading"`. |
| Header | `Preview — v{N} · {timestamp}` Inter 11px `--color-text-muted` uppercase, letter-spacing 0.02em |
| Sections | One per non-empty field: Summary, Prose, Notes. Section label Inter 600 10px uppercase `--color-text-muted` letter-spacing 0.04em. Each section has `data-testid="version-preview-section-{summary\|prose\|notes}"`. |
| Section body | Rendered inside `<div data-editor="{summary\|prose\|notes}"><div class="tiptap">` so the typeface rules in `app/globals.css` apply unchanged — Inter 13px for summary + notes, Lora at `--text-prose-panel` for prose. |
| Padding / border | Pane wraps in `--color-bg-base` background, 1px `--color-border-subtle` border, `--radius-sm`. |
| All-empty content | "This version has no summary, prose, or notes content." italic, `--color-text-muted`. `data-testid="version-preview-empty-content"`. |

**Renderer (`renderTiptapJson`).** Recursive walk of Tiptap JSON producing React nodes — pure function, exported for unit-test. Supported blocks: paragraph, heading (level 1-6, clamped), bulletList, orderedList, listItem, blockquote, horizontalRule, hardBreak. Supported inline marks: bold (`<strong>`), italic (`<em>`), link (`<a target="_blank" rel="noreferrer noopener">`), code. Unknown blocks fall through to rendering their children (text never lost); unknown marks are silently skipped (text still renders unmarked). Unparseable JSON falls back to plain-text rendering inside `<p>`. React's default attribute escaping is preserved — no raw `<script>` ever reaches the DOM (`tests/unit/version-preview-renderer.test.ts` includes a regression guard).

🔒 **No editor.** The pane MUST NOT mount a Tiptap editor — the renderer walks JSON directly. Mounting a read-only Tiptap instance per selection change would re-create an editor for every click, and the preview doesn't need editor behaviour (no cursor, no toolbar, no autosave, no Inviolable-#5 concerns).

🔒 **Typeface boundary preserved.** Summary and Notes preview sections render in Inter; Prose preview renders in Lora. This is enforced by reusing the `data-editor="…"` selector pattern from `app/globals.css` — no duplicate typeface declarations in the preview component itself, so any future change to the editor typeface scale propagates here automatically.
