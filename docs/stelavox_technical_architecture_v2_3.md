...
> **Director architecture (V2):** §8.2–§8.5 in this document are superseded by `docs/stelavox_director_architecture_v2_0.md` (Tier-A canonical). The headings remain for navigation; their content now references the V2 doc. §8.1 (configuration-driven architecture invariant) and §8.6 (config version lifecycle) are retained from v2.2. Hazards H-16 through H-20 are introduced by the V2 design and listed in §5.
...
| V1.x-A | **Brief + Stage substrate** (`docs/stelavox_director_architecture_v2_0.md` §6, §16). Migrations for `briefs`, `brief_stages`, `brief_amendments`; `documents.brief_id` FK; `lib/brief/` module; `get_brief_state` read tool; `propose_brief_amendment` + `<brief_proposal>` proposal shapes; `BriefViewer` + `StageCard` components; per-iteration Director execution model with Realtime-channel client updates. | post-V1.x-LB | Author can approve a Brief; stages advance via scheduler invoking Director; per-iteration recovery works end-to-end. |
| V1.x-B | **Scheduler + throttle redesign.** Scheduler as universal coordinator (no special cases). Queue tables, traffic classes, WFQ, per-user buckets, Class-1 reserved capacity, reservation TTL cleanup (H-17), recovery sweep. `batched_24h` execution intent wired to Anthropic Batch API with webhook callback. Replaces cap=1 holding pattern. `get_scheduler_state` read tool. | post-V1.x-A | Multi-workflow auto-split runs without 429s; queues observable; admin dashboard shows class-by-class throughput. |
