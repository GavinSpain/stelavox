...
> **Model Governance SHIPPED 2026-06-16 — bulletproof metering + admin model management.** **Migration count 229 → 233** (M-229 = Phase 13.1 waitlist). Closes a revenue-leakage hazard surfaced during the founding-discount review: a model assigned to a tool/Director without a pricing row metered nothing (platform pays Anthropic, customer charged nothing). The leak failed open at three layers — free-text model columns, fail-open admission gate, NULL `cost_credits` on completion. **§3.5 schema:** new `llm_models` registry (model_id PK · display_name · provider · status active/deprecated/hidden); **M-230** seeds Haiku 4.5 / Sonnet 4.6 / Opus 4.7 (deprecated) / **Opus 4.8** (active) + FK `anthropic_pricing.model_id → llm_models`; **M-231** collapses `pricing_rates` to a **derived VIEW** of `anthropic_pricing` (credits = dollars × 1,000,000 — the two can no longer diverge; one source of truth); **M-232** `is_model_assignable()` + FK + BEFORE-INSERT/UPDATE triggers on `agent_profiles.model_id` + `director_configs.model_id` reject an unknown/deprecated/unpriced assignment; **M-233** fallback debit config + `audit_metering_integrity()` backstop + deletes the dead `price.anthropic.*` keys. **§7.5 admission gate** now FAILS CLOSED on a missing rate (refuse + critical audit). **§3.6 completion** uses `lib/cost/completionCredits.ts` `computeCompletionCredits` — guaranteed non-null (exact when priced; conservative fallback debit + critical audit when not — never free); wired into both `runner.ts` paths + `iteration-runner.ts`. **§A.5 admin** gains `/admin/models` (registry CRUD + re-price + activate/deprecate + an assignment matrix for the Director + 22 agent tools, dropdowns constrained to active+priced models) via `GET/POST /api/admin/models` + `lib/admin/models/registry.ts`; the Operations Summary gains a `metering_integrity` band. **New hazard H-29 — unmetered-model revenue leakage** (mitigation = the registry + integrity triggers + fail-closed gate + non-null completion + backstop; full §5 entry to be authored at the next TA consolidation; summary in CLAUDE.md §Known-Hazards). **Verification:** type-check + lint + production build clean; DB integrity proven via psql + 11 new Vitest (code-layer fallback/gate + DB-layer triggers/view/backstop); the pre-existing `v1x-c2-budget-gate` "missing rate passes through" case reversed to fail-closed. **No Inviolable changes; admin adds zero verdigris.** **Tier-A propagation:** Director Architecture unchanged; CLAUDE.md v1.58 → v1.59; this TA v2.25 → v2.26. Deferred: `supabase gen types` regen (new objects accessed only via the untyped service-role client).

> **Phase 10 SHIPPED 2026-06-14 — the final three V1 folds; all 22 V1 deliverables now shipped.** **Migration count 224 → 228.** **DR-042 export rework** — M-225 adds `export_jobs.root_node_id` (subtree-scoped per-book export) + `file_name` (download name) + a Markdown manuscript built-in profile, removes the JSON built-in profile, and extends `export_profiles.format` CHECK to admit `markdown`. The tree-walker gains optional `rootNodeId` scoping; new `lib/export/markdown.ts` renders the structure-+-final-prose backstop; `POST /api/exports` accepts `book_node_ids[]` and fans out one job per Book (each scoped via `root_node_id`, filename `{Series} — {NN} {Book}.{ext}`); `lib/export/json.ts` deleted; `ExportFormat` is now `docx|epub|markdown|outline`. **DR-057/058 Inviolable #2 audit** — two uncatalogued decorative verdigris dots in YoursTile → neutral; mentioned-node tree-row border recorded as use #9's second function; SearchChip hover-state verdigris (a §5.4 violation) reverted to neutral. Verdigris-use count remains 12. **DR-121 Admin Operations Summary** — M-226 `export_jobs.file_size_bytes` (storage aggregation) + M-227 `admin_ops_infra_health()` SECURITY DEFINER RPC (reads cron / net._http_response / pg_publication_tables that PostgREST can't reach; `SET search_path = public` per H-13) + M-228 `bump_conversation_window_pressure()` proxy. New `GET /api/admin/ops` returns a 7-band feed (health rollup · liveness heartbeats · external dependencies · volume/growth · export/storage · plan economics · LLM provider reconciliation) almost entirely derived — the per-model platform spend `SUM(agent_jobs.cost_usd) WHERE route='platform' GROUP BY model_id` IS the Anthropic invoice. `OperationsSummary` mounts atop `AdminDashboard`; the runner persists file size; the context-builder fires window-pressure fire-and-forget; queue-age + realtime-health derive live. **No new hazards** — the infra RPC is read-only + exception-wrapped per source; window-pressure is fire-and-forget; the subtree walk is pure. **No Inviolable changes.** **Tier-A propagation:** Director Architecture unchanged; Product Spec to note the export lineup + admin ops at the next consolidation; CLAUDE.md v1.57 → v1.58; V1 Deliverables Register v3.5 → v3.6 (V1 remaining 3 → 0).

> **Phase 9.E SHIPPED 2026-06-13 — work package E (UX finish-the-job batch; DR-020/043/046/050/066).** §3.7 platform_config gains 1 key via **M-224** `failure.injection_blocked_message` (admin-tunable security-warning copy for DR-050). **Migration count moves 223 → 224.** This is a UI-adoption phase — substrate for all five items shipped in earlier phases (V1.x-F failure components, Phase 6 lock RPCs, Phase 5b interrupted-turn surfacing); 9.E mounts/wires them. **DR-020:** new `components/feedback/FailureSurface.tsx` wrapper + `lib/ui/classifyFailure.ts` (HTTP status → V1.x-F failure class A/C-toast / D/E-banner) + GET `/api/failure-messages` (auth-gated, serves the admin-tunable bundle so client surfaces never hardcode copy — H-12). Adopted in `AgentTab` (6 setError→setFailure sites) + `SchedulerPanel`. AppShell-global is satisfied by the existing RealtimeBadge. The classifier returns null for 423 and 422-injection so the dedicated DR-046 / DR-050 surfaces own them with no double-render. **DR-046:** refined the existing `ConflictBanner` (already mounted, already consuming editor-store `lockedReason`) lock branch — distinguishes author-lock from parent-lock, "Reload" label (vs "Use latest" on 409), "⋯ → Unlock" guidance, "edits up to the lock are saved" reassurance. **DR-050:** new `components/feedback/SecurityWarningBanner.tsx` on the FailureBanner chassis (`--color-status-review` caution border, ⛊, copy from M-224 via the bundle); `isInjectionBlockedError()` recognises the runner-persisted `injection_blocked:<field>`; **NO override affordance anywhere** per the author lock; AgentTab's stale "override on the roadmap" friendlyError line removed. **DR-066:** threaded the conversation GET's pre-existing `interrupted_message_id` (Phase 5b I-12) through `useDirectorConversation` (`interruptedMessageId`) and mounted `StoppedFollowOnBanner variant="interrupted"` in DirectorPanel; Resume/Cancel/View + resume endpoint shared with the V1.x-D stopped variant. **DR-043:** new `components/tree/BulkUnlockConfirmModal.tsx` + `NodeMoreMenu` reads the target author-lock's `bulk_operation_id` + batch count from `node_author_locks` (RLS org-member read) and offers unlock-all (DELETE bulk-operation route) vs unlock-one (DELETE single lock route). **Verification:** type-check clean; new/extended Vitest (classify-failure 10, injection-warning, phase6b DR-043 membership cases) PASS; sole full-suite failure is the documented `m173-m174-h26-audit #12` local-DB-state flake (no director_turns rollup code touched). **No new hazards** — the classifier is pure; FailureSurface degrades to null when the bundle is unavailable; SecurityWarningBanner has no override path by construction. **No Inviolable changes; verdigris-use count remains 12** (all feedback surfaces use neutral / status-review / error tokens). **Tier-A propagation:** Director Architecture unchanged; CLAUDE.md v1.56 → v1.57; V1 Deliverables Register v3.2 → v3.3 (DR-020/043/046/050/066 badged SHIPPED; V1 shipped 14 → 19, V1 remaining 8 → 3). **All five V1 work packages (A–E) are now SHIPPED; only the 3 Phase-10 folds remain.**

> **Phase 9.B admin payments SHIPPED 2026-06-13 (DR-070 follow-up).** §3.7 platform_config gains 4 new keys via M-223: `stripe.api_version` (extracts the only remaining hardcoded payment parameter from `lib/stripe/client.ts:17`), `stripe.checkout.automatic_tax_enabled` (default false), `stripe.checkout.allow_promotion_codes` (default false), `stripe.checkout.billing_address_collection` (default `'auto'`). Migration count moves 222 → 223. **The "no hardcoded parameters" stance is now structurally enforced**: drift-guard Vitest `tests/unit/stripe-api-version-no-hardcode.test.ts` forbids re-introduction of `apiVersion: '...'` literals in `lib/stripe/`. **Past-due gate (D4.c, defence-in-depth):** the credit gate (`lib/llm/token-budget.ts`) refuses Director/agent dispatch immediately when `organisations.subscription_status='past_due'`; the layout (`app/(app)/layout.tsx`) surfaces a thin global banner in AppShell + a prominent banner on `/settings/plan` with copy reading the grace window from `billing.payment_failure_grace_days`. Author lock: "no LLM access without a valid subscription"; app + writing access preserved during Stripe's Smart Retries window. **3 new webhook events** wired into `HANDLED_EVENT_TYPES`: `charge.dispute.created` (severity='critical' per D5.a), `charge.refunded` (severity='high'), `invoice.payment_action_required` (3DS / SCA, severity='high'); each writes `audit_log` with full metadata; no auto-action. **Admin `/admin/payments` page** at `app/(app)/admin/payments/page.tsx` + `components/admin/AdminPayments.tsx`: 5 tabs (Configuration · Price IDs · Subscription health · Events · Failures); URL state via `?tab=` query param; `PLATFORM_ADMIN_EMAILS` env-var allowlist auth (matches V1.x-E `/admin`); Inter only, no verdigris. The /admin dashboard gains a deep-link to /admin/payments. **Inline-edit writes (D1.a webhook double-entry):** `lib/admin/payments/writes.ts` with per-key validators (enum, bool, int-range, api_version, price_id, webhook_secret); POST `/api/admin/payments/config` accepts `{ key, value, valueConfirm? }`; every successful write triggers `audit_log` (`event_type='admin_payments_config_changed'`, severity='medium', `{key, old_value, new_value, admin_user_id}`); webhook-secret writes mask values in audit storage; `_clearConfigCache()` + `_clearStripeClientCache()` after writes that affect the SDK client (mode + api_version). **Verification:** 5 new Vitest files / 26 new cases — `admin-payments-data` (5: section shape), `admin-payments-writes` (12: validators + audit + double-entry + no-op cache behaviour), `past-due-gate` (4: status-aware credit gate), `stripe-api-version-no-hardcode` (1: drift guard), `stripe-webhook-handlers` extension (3: dispute + refund + payment_action_required). Type-check clean. Cross-file `platform_config` races are absorbed via tolerant assertions (matches the pattern documented in v1.54 / v1.55 changelog entries). **No new hazards** — the SDK client cache is invalidated on mode/api_version writes; the past-due gate is layered (credit gate + layout banner); webhook handlers remain idempotent at the M-221 unique index. **No Inviolable changes; verdigris-use count remains 12.** AdminPayments and the past-due banners use neutral / status-review tokens only. **Tier-A propagation:** Director Architecture unchanged (Stripe out of scope); CLAUDE.md v1.55 → v1.56; V1 Deliverables Register DR-070 closeout extended (no bucket change — still SHIPPED).

## Version 2.22

> **Phase 9.B yearly cadence (DR-070 follow-up; SHIPPED 2026-06-11).** §3.7 platform_config gains 8 yearly Price ID slots (`stripe.{test,live}.price_id.{writer,author,pro,byok_solo}_yearly`) via M-222 — total Stripe Price ID slots double from 8 → 16, all empty until the Stripe account is provisioned. **Allocation cadence locked**: credits reset PER MONTH regardless of cadence. The existing `rollover_org_periods()` pg_cron job (M-141) is unchanged; annual subscribers get monthly credit refreshes, only the Stripe billing cycle is annual. No schema change to `organisations`; no new column. **lib/stripe**: `Cadence = 'monthly' | 'yearly'` + `STRIPE_CADENCES` + `DEFAULT_CADENCE='monthly'` exported from `lib/stripe/config.ts`; `getStripePriceId(mode, plan, cadence)`; `requireStripeConfigured` validates ALL 8 Price IDs per active mode (8 from M-219 + 8 from M-222) — webhooks can fire on either cadence so neither set can be partially missing. `createCheckoutSession({plan, cadence})`; `priceIdToPlan` now returns `{plan, mode, cadence}`. **API route**: `/api/billing/checkout` accepts `{plan, cadence?}` with `cadence` validated against `STRIPE_CADENCES`; defaults to monthly when absent (back-compat). **Webhook handler**: `handleSubscriptionCreatedOrUpdated` surfaces `cadence` in the audit metadata via `priceIdToPlan`; allocation logic unchanged (per-month resets, both cadences). **UI**: PlanPanel becomes a client component; new `CadenceToggle` ("Monthly · Annual (save 20%)") at top, default monthly; selected cadence drives the price prominently shown on each TierCard + the Price ID that Subscribe POSTs. The alternate-cadence price displays as the secondary line ("$X/mo monthly" when annual is selected, "$Y/yr (–20%)" when monthly is selected). SubscribeButton accepts a `cadence` prop. **Decisions locked 2026-06-11 with author:** per-month resets even for annual subscribers (matches Notion / Anthropic SaaS norm); monthly is the default UI cadence (lower commitment friction; annual discoverable via the toggle). **Verification:** 35 Phase 9.B Vitest cases PASS (10 stripe-config incl. new STRIPE_CADENCES tests + 12 trial-expiry + 9 stripe-plans incl. new yearly Price ID mapping + 4 stripe-webhook-handlers with cadence audit assertion). 4 stability runs confirm no cross-file race. **Migration count moves 221 → 222.** **Setup checklist update:** §6 of `stelavox_phase9_b_session1_test_report_v1_0.md` now requires 8 Price IDs per mode (4 monthly + 4 yearly); the 4 yearly Prices are created as additional Price rows on the existing Products (Stripe model: one Product, multiple Prices for cadence/currency variants). **No new hazards. No Inviolable changes; verdigris-use count remains 12.** CadenceToggle uses neutral styling (no verdigris). **Tier-A propagation:** Director Architecture unchanged; CLAUDE.md v1.54 → v1.55; V1 Deliverables Register: DR-070 closeout unchanged (still SHIPPED; this is a scope expansion within the same DR).

## Version 2.21

> **Phase 9.B SHIPPED 2026-06-11 (DR-070 — Stripe integration).** §3.5 schema gains `organisations.{trial_expires_at, stripe_price_id}` (M-220) + `subscription_events` partial unique index on `stripe_event_id` (M-221) + 11 new `stripe.*` platform_config keys (M-219). §3.7 platform_config table updated with the new keys (`stripe.mode` + 4 test-mode Price IDs + 4 live-mode Price IDs + `stripe.webhook_secret_{test,live}`). Migration count moves 218 → 221. **`stripe` npm package added** (22.2.0; apiVersion `2026-05-27.dahlia`). **Library + routes:** `lib/stripe/{config,client,customers,sessions,plans,webhook-handlers}.ts` + `lib/billing/trialExpiry.ts` + POST `/api/billing/checkout` + POST `/api/billing/portal` + POST `/api/stripe/webhook`. **Substrate behavior locks:** test mode is default at launch (swap to live via `stripe.mode` UPDATE — no deploy); `STRIPE_SECRET_KEY_{TEST,LIVE}` in env vars (auth credential); price IDs + webhook secrets in platform_config (DB-auditable); all routes return 503 `stripe_not_configured` with explicit `missing` list when any piece is blank — the substrate ships safely before the Stripe account is provisioned. **Trial-expiry gate:** `(app)/layout.tsx` reads `(plan, trial_expires_at)` and redirects to `/settings/plan?reason=trial_expired` when an expired-trial org navigates to any non-exempt path (`/settings/plan*`, `/api/billing/*`, `/api/stripe/*` are exempt). Per the product lock: "credit exhaustion never blocks writing" applies to paid plans only — trial users hit the expiry redirect when time runs out; paid users only hit the credit gate (which already refuses Director/agent dispatch without blocking manual prose). **Webhook handler:** 11 event types subscribed (checkout.session.completed · customer.subscription.{created,updated,deleted,trial_will_end} · customer.{created,updated} · invoice.{payment_succeeded,payment_failed,upcoming} · payment_method.attached); signature verified via `stripe.webhooks.constructEventAsync` against `stripe.webhook_secret_<mode>`; idempotency via M-221 unique index + 23505-swallow on duplicate INSERT; per-event handlers sync `organisations.{plan, subscription_status, stripe_subscription_id, stripe_price_id, current_period_start, byok_enabled, token_allocation_credits}` — plan slug resolved by reverse-lookup `priceIdToPlan` over the 8 configured Price ID keys; American↔British spelling normalised (`canceled`→`cancelled`, `trialing`→`trialling`). **UI:** SubscribeButton + ManageSubscriptionButton client components mounted in PlanPanel; PlanPanel V1.x-D "No Stripe Checkout in V1" note replaced by an active Manage subscription section gated on `hasStripeCustomer`; `/settings/plan` shows Stripe-status banner on `?stripe_status=success`/`cancelled` and a trial-expired banner on `?reason=trial_expired`. **Verification:** 32 new Vitest cases across 4 files (stripe-config 10 + trial-expiry 12 + stripe-plans 6 + stripe-webhook-handlers 4) PASS. Type-check clean. End-to-end activation gated by the user-driven Stripe account setup — the checklist lives in `docs/stelavox_phase9_b_session1_test_report_v1_0.md` §6. **§5 hazards reviewed** — no new H-NN: the substrate-uninitialised state is the explicit deployment path with a structured 503 response; the trial-expiry redirect can't loop because `isPathTrialExempt` excludes the destination; the webhook handler is idempotent at the DB level. **No Inviolable changes; verdigris-use count remains 12.** SubscribeButton uses verdigris affordance under the existing use #7 affirmative-action family (no broadening). **V1 hard-blocker list drops 9 → 8** (DR-070 closed). **Tier-A propagation:** Director Architecture unchanged (Stripe is out of scope); Product Spec v1.19 → v1.20 (§3.1 V1 plan lineup + Subscribe / Manage Subscription flows now real; §3.2 trial-expiry behavior locked: redirect to plan-buy page, no data loss); CLAUDE.md v1.53 → v1.54; V1 Deliverables Register DR-070 badged ✅ SHIPPED 2026-06-11.

## Version 2.20

> **Phase 9 work package D — DR-051 SHIPPED 2026-06-11.** §3.6 Director-config row updates: production version moves to **v1.29** ("auto-create-and-link context teaching"). M-218 deprecates v1.28 and inserts v1.29 with the prompt rewritten in exactly two passages — the `generate_context` Step shapes line (documents `seed_content` and admits structural targets) and the Context-generation specialist paragraph (describes the auto-create + `structural_to_context` link path). Tool registry preserved at 17 (12 read + 5 write). Migration count moves 217 → 218. **Director-config back-documentation:** the work package D apply pass surfaced that v1.25/v1.26/v1.27/v1.28 had shipped without changelog entries — M-186 v1.25 "drop the expand `instruction` parameter drift", M-187 v1.26 "align step shapes", M-208 v1.27 "completeness model", M-209 v1.28 "inferential discipline". The prompt-version chain now reflects v1.24 → v1.25 → v1.26 → v1.27 → v1.28 → v1.29. **Substrate already shipped** — the executor's auto-create + auto-link code at `lib/director/workflow-executor.ts:496-578` has been live since 2026-05-08 (Mars-series cloud finding); DR-051 was a prompt + documentation gap, not a substrate gap. **Verification:** 3 new integration cases in `tests/unit/workflow-executor-context.test.ts` exercise the executor's auto-create + link + re-target path against a real local DB; 1 new drift-guard case in `tests/unit/director-prompt-vs-schema-drift.test.ts` pins v1.29's new framing and asserts the pre-v1.29 "context node must pre-exist" paragraph is gone. **Latent follow-up flagged in SU-J11-2:** the executor's `transitionWorkflowStep` call when `context_type` is missing fires from `pending` but the only legal transition to `workflow_steps.failed` is from `running` (post-M-205 Apollo CAS sweep) — the CAS loses silently. Separate small executor bug; auto-create artefacts remain correctly absent because the early-return runs first. **§5 hazards reviewed** — no new H-NN. **No Inviolable changes; verdigris-use count remains 12.** **Tier-A propagation:** Director Architecture v2.9 → v2.10; V1 Deliverables Register: DR-051 badged ✅ SHIPPED 2026-06-11; CLAUDE.md v1.52 → v1.53. V1 hard-blocker list drops 10 → 9.

## Version 2.19

> **Phase 9.1 + 9.2 SHIPPED 2026-06-11.** §3.5 migration count moves 214 → 217 (M-215 `increment_usage_record_rpc` atomic UPSERT closes the F-133/F-134 race; M-216 `pg_net_cloud_dispatch` adds `invoke_scheduler_endpoint` + completion-trigger HTTP push + new AFTER-INSERT enqueue trigger + cadence-function HTTP invokes for batch / route-sample / probes + 1-minute `dispatcher_sweep_http` pg_cron job; M-217 hotfix bumps `pg_net.http_post` timeout 5000 → 30000ms after the first cloud invocation surfaced timeout-masking-of-real-errors noise). Plus 7 May-2026 migrations restamped (sequence-numbers 142..148 had `20260518`/`20260519` date stamps that contradicted their sequence numbers; filename order != dependency order; reset-replay failed at M-164 with NULL on the missing v1.10 config M-146 had not yet created) — pure rename, semantic content unchanged. **§11 Phase 9.1 + 9.2 rows added** documenting the 12 V1 register items closed (DR-063 / DR-067 / DR-095 / DR-096 / DR-097 / DR-098 / DR-100 / DR-102 / DR-103 / DR-111 / DR-112 / DR-117). **§5 hazards reviewed** — no new H-NN: the pg_net helper is double-gated (`scheduler.cloud_dispatch_base_url` must be set + Vault `cron_secret` must exist), every failure path is RAISE WARNING (cannot abort caller's transaction); the cloud listener absence is by-design and not a hazard because the cron sweep is the safety floor. **No Inviolable changes; verdigris-use count remains 12.** Cloud dispatch transport activated against stelavox-dev (`scheduler.cloud_dispatch_base_url = "https://stelavox.vercel.app"`); `dispatcher_sweep_http` + `route-sample` cron jobs firing autonomously every minute and recording 200 OK in `net._http_response`. **Director Architecture v2.8 → v2.9** documents Option E as the canonical cloud-dispatch transport (replaces the old "listener on always-on worker" V1.x backlog item). CLAUDE.md v1.51 → v1.52. **Test Report:** `stelavox_phase9_1_2_test_report_v1_0.md` PASS. **V1 hard-blocker list reduced from 22 to 10** — remaining: work package B (Stripe, DR-070), D (Director context, DR-051 — needs Tier-B design), E (UX batch × 5: DR-020/043/046/050/066), plus 3 Phase-10 folds.

## Version 2.18

> **Phase 9 — pre-launch backlog review 9.A–9.D MET (2026-06-10):** **§11 Phase 9 row updated; §11 Backlog row superseded by the V1 Deliverables Register.** The full project reset inventoried every deferred / queued / V2-V3-tagged item (Director Architecture §16.3 + §16.4, this doc's Backlog row, per-phase memos, Round-3 audit residual at theme level, open SUs, CLAUDE.md changelog) into `stelavox_v1_deliverables_register_v1_0.md` — a NEW Tier-A planning artefact that is the single source of truth for undone work. 121 items (DR-001..DR-121) across 11 categories (Agent System / Document Operations / Author UX / Brand / Data+Infra / LLM Cost / Failure UX + Observability / Multi-tenancy / Deployment / Documentation / Security + 14 Round-3 audit themes), ALL bucket-assigned by the author in one review session: **22 V1** (five work packages — A cloud cutover, B Stripe, C security + correctness hardening, D Director context create+link, E UX finish-the-job — plus 3 folds into Phase 10) · ~42 V1.x · ~40 V2+ · 2 V3 · 2 dropped. **Product-scope locks:** V1 plan lineup = free trial month + writer/author/pro single-user platform plans + `byok_solo` (`byok_team` dormant until V2 multi-user orgs); Stripe is V1 with BYOK paying a platform fee; credit exhaustion locks Director only — never writing; V1 = single-user orgs; no injection-scanner override (security-warning UX only); rate-limit fail-closed; Stripe hosted Customer Portal for subscription management. **No code change, no migrations, no new hazards, no Inviolable changes** — pure planning consolidation. Remaining: 9.E work-package sequencing. CLAUDE.md v1.50 → **v1.51**.

> **M-210 — node_versions integrity (2026-05-24):** **§3.5 migration count moves 209 → 210** (+1 — M-210 `node_versions_version_integrity.sql`). Two stacked root causes produced duplicate `(node_id, version)` snapshot rows in the live DB, surfaced when the v1.42 VersionHistory click-to-preview pane tried to load a version via `.maybeSingle()` and errored on the duplicates. (1) The M-035 trigger required BOTH the GUC `stelavox.bump_version='true'` AND a real content delta to bump `nodes.version` — when `accept_agent_job` applied content byte-identical to the existing prose, the trigger silently skipped the bump despite the explicit GUC signal, and the unconditional snapshot INSERT then stacked another row at the unchanged version number. (2) `accept_agent_job` unconditionally inserted a snapshot for every accepted job — including `expand`, which per M-034 doesn't modify parent content; repeated expand thus produced multiple snapshots at the same version with byte-identical content. M-210 lands six in-transaction steps: renumber duplicate snapshots by `created_at ASC` (5 affected nodes, 10 rows renumbered); reconcile `nodes.version = MAX(snapshot.version) + 1` for affected nodes (68 rows touched); `ALTER TABLE node_versions ADD CONSTRAINT node_versions_node_id_version_unique UNIQUE (node_id, version)` as structural backstop; rewrite the M-035 trigger to GUC-first logic (when the GUC is set, always bump both `version` and `content_revision` regardless of content delta — the caller's intent is the bump signal, not the byte diff); rewrite `accept_agent_job` to skip the snapshot INSERT for `expand`; bypass triggers during backfill via `SET LOCAL session_replication_role = replica`. **§5 Hazards adds H-28 — Version-bump trigger short-circuits on identical content** (full entry below). Pre-M-035 the trigger never accepted a "force bump" semantic; M-210 makes the GUC the single source of truth for version-bump intent for both `accept_agent_job` (synth/refine/generate_context paths) and `restore_node_version`. **Autosave invariant preserved:** the GUC-unset branch still bumps `content_revision` only on real content change; TC-A-11 ("Same content does not bump version") still passes. **Tests: 6/6 new Vitest** (`m210-version-bump-on-guc.test.ts` — GUC+changed / GUC+identical / autosave+changed / autosave+identical / UNIQUE backstop / expand skips snapshot). Phase 6.C restore fixture updated to use a real accept_agent_job pass-through (the previous setup pre-inserted a v=1 history row while leaving nodes.version=1 — inconsistent, but pre-M-210 the unique constraint didn't enforce it). **Vitest: 640/650 PASS** (8 pre-existing failures unchanged, 10 baseline skipped). **No Tier-A doc bump beyond TA + CLAUDE.md** — pure backend integrity fix below the UI layer; the v1.42 click-to-preview pane is now functional with no API change (`getVersion` continues to use `.maybeSingle()`, which is correct now that duplicates can no longer exist). CLAUDE.md v1.42 → **v1.43**. **Five Inviolables intact; verdigris-use count remains nine.**

> **Director simplification — amendment surface removed (2026-05-21):** Four schema-vs-code drift bugs in 48 hours of testing the V1.x-B.3 `propose_brief_amendment` surface motivated a focused refactor. The amendment tool + table + RPC + UI card are all removed; the dominant use case (planning a stage's workflow when its trigger fires) is now handled by a new `propose_workflow` tool the system invokes via the same `stage_trigger_fired` push-model event. **§3.5 migration count moves 163 → 165** (+2 — M-183 `brief_simplification.sql` drops `brief_amendments` table + `apply_brief_amendment` RPC, adds `brief_stages.prompt TEXT`, adds CHECK constraint `brief_stages_planning_source_check` requiring workflow_id-or-prompt on non-terminal stages, narrows `brief_stages.trigger_type` CHECK to `('after_stage','manual')` (drops scheduled_at + compound — never exercised in V1.x), narrows `brief_stages.status` CHECK with `'proposing'` renamed to `'planning'`, restores the strict-one-active partial unique index on `briefs(document_id) WHERE status IN ('planned','active')` that M-126 had dropped for V1.x-B.3 multi-active. M-184 `director_v1_24_simplification.sql` deprecates director_config v1.23 and inserts v1.24 production — tool_suite swaps `propose_brief_amendment` for `propose_workflow` (count unchanged at 17), system_prompt rewritten in Briefs / Tools / Plan-before-you-propose / System-initiated-turns sections; other sections preserved from v1.23. M-185 `evaluate_ready_stage_triggers_v2.sql` rewrites the push-model evaluator to use `status='planning'`, skip stages whose workflow_id is already set, include the stage's stored prompt in the director_iteration's initial user message, and pin system_prompt_version='1.24'. **Brief model change:** each `brief_stages` row is now either (a) workflow-bound at proposal time (workflow_id set, prompt NULL) or (b) prompt-deferred (workflow_id NULL, prompt set; the system invokes the Director with the prompt when the trigger fires; the Director responds via `propose_workflow`; the iteration runner persists the workflow, links it to the stage, transitions status back to `'planned'`, and auto-approves+dispatches if `brief.auto_approve_workflow_proposals=true`). **Multi-active brief support (V1.x-B.3) dropped** — one active brief per document, matching the simpler default scope. **§5 Hazards:** No new hazards. The new model removes failure modes (no amendment-tool drift surface, no auto-approve half-measure, no dual-card approval path). `brief_amendments` table no longer exists; the section of §3.5 describing it should be read as historical context only. Pre-migration cleanup: all scheduler/brief tables were truncated to clean state; author writing data (nodes, node_versions, projects, documents, context_*) was preserved. Snapshot at `snapshots/stelavox_local_2026-05-21_pre_simplification.dump`. **Tier-A docs all bumped in lockstep:** TA v2.14 → v2.15 (this entry); Director Architecture v2.6 → v2.7; Component Spec v2.17 → v2.18 (drops BriefAmendmentCard, BriefProposalCard renders stage prompts); Product Spec v1.16 → v1.17 (one-active-brief + no interim approvals on auto-approved briefs); CLAUDE.md → v1.39. **Test suite:** -3 obsolete test files (`v1x-b3-amendment-validators`, `propose-brief-amendment-executor`, `multibrief-and-amendments`); +1 new (`propose-workflow-executor`, 7 cases against real local DB). Unit suite: 594 passed / 0 failed. **Author rationale (verbatim from the session, 2026-05-21):** *"If it is this hard for the llm to get right and honestly im having trouble understanding it myself, then my concern is that its not an elegant solution. We need a simple queue based process flow that is bulletproof. ... It is simplicity that will deliver robustness. ... we need predictability and that will come through simplicity."*

> **Word-count trigger + derived-field-staleness hazard family (2026-05-18):** §3.5 migration count moves 162 → 163 (+1 — M-172 `word_count_actual_trigger.sql` — `tiptap_word_count(JSONB)` IMMUTABLE SQL helper walks Tiptap doc tree + whitespace-counts; `compute_node_word_count_actual()` BEFORE INSERT OR UPDATE trigger on `nodes` recomputes `word_count_actual` when prose changes; backfill UPDATE for the 459 existing rows). §5 Hazards gains **H-26 — Derived-field staleness** (full entry below): column added to a table to hold a *derived* value but no maintenance mechanism wired up; column ships populated at insert time, then every subsequent update leaves the derived column NULL or stale; invisible unless a programmatic consumer (an agent, an export, an analytics query) starts reading the value. First instance: `nodes.word_count_actual` — defined in M-004, never maintained until M-172, surfaced 2026-05-18 when Director testing reported NULL counts on 37 beats that all had prose. Two sibling invariants audited 2026-05-18, both holding in current data, both now under layer-3 test: `nodes.version - 1 = COUNT(node_versions WHERE node_id=N)` (maintained by M-035 + M-090 GUC trigger), and `briefs.current_stage_id` ↔ `brief_stages.status` (maintained by `complete_brief_stage_workflow` + `propagate_brief_completion`). H-26 anticipates more instances; full audit deferred but the pattern is now formal. **Testing methodology change** (separate memory file `feedback_testing_methodology.md`): four-layer methodology formalised — layer 1 pure-function unit tests, layer 2 tool contract tests for every Director read/write tool (≥6 cases: empty/single/multi/cap/error/null), layer 3 invariant tests for every derived field, layer 4 AI-led big-picture pass answering 6 structured questions (spec alignment / invariant inventory / consumer map / hazard register / failure modes / "what would I miss?"). Trigger conditions for layer 4 specifically: any Director tool change, any write path to high-leverage tables (`nodes` / `agent_jobs` / `briefs` / `brief_stages` / `conversations` / `director_turns`), any new SQL function or trigger, any schema migration, any security-primitive change, any change touching the hazards list. M-172 is the first instance shipped under the new methodology — 11 layer-1 tests (tiptap_word_count: null / empty doc / whitespace-only / single + multi word / marked text / unicode em-dash / large doc / plain string scalar) + 6 layer-3 invariant tests (INSERT with/without prose / UPDATE prose recomputed / UPDATE non-prose NOT recomputed / prose→NULL / prose→empty doc / backfill correctness sweep) + layer-4 pass (surfaced this absorption block + H-26 + the sibling audits + a cross-language parity test follow-up between TS `extractText`/`countWords` and SQL `tiptap_word_count`). **Cross-language algorithm parity** flagged as a related concern but deferred — the right antibody is a corpus parity test, not a rewrite. **Five Inviolables unchanged. No verdigris additions.** **Related Tier-A docs not bumped** (no consumer surface change; the Director read tools already had `word_count_actual` in their return shape, the trigger only makes the value reliable).

> **Phase 7 close-out absorption (2026-05-17):** §3.5 migration count moves 157 → 162 (+5 for Phase 7 — M-158 `export_jobs_extensions.sql` (progress JSONB + profile_id FK + last_active_at + cancellation_requested_at + attempt_count + total_chapters; 7-state queue_status enum: queued → planning → rendering → assembling → uploading → completed | failed | cancelled; Realtime publication; recovery index); M-159 `export_profiles_table.sql` (CHECK constraint forces built-in (NULL org+project) OR author-saved (both NOT NULL); 4 built-in seeds — DOCX-Manuscript, DOCX-KDP, EPUB-Standard, Outline-Structural); M-160 `export_profile_rpcs.sql` (save_export_profile / update_export_profile / delete_export_profile SECURITY DEFINER); M-161 `export_platform_config.sql` (6 keys — signed_url_ttl_hours 168, max_words_per_document 1,500,000, max_chapters_per_document 500, max_render_minutes 4, max_file_size_mb 50, soft_warning_words 900,000); M-162 `export_retention_cron.sql` (purge_expired_exports daily 03:30 UTC + recovery_sweep_exports every minute). §3.6 schema register documents the 7-state pipeline + per-project `export_profiles` table with built-in/author-saved split. §3.7.4 Canonical Configuration Keys gains the `export.*` family (6 keys). **§9 Export Pipeline** updated for Option B atomization (resumable per-chapter pipeline; cancellation check at chapter boundaries; renderer dynamic-import; Realtime DB subscription for progress, not SSE). **§11 Phase Plan** Phase 7 row checkpoint MET. Phase 7 ships in four sub-phases on branch `claude/lucid-kare-fc8032` merged to master via `--no-ff`: 7.A Foundation (M-158/159/160/161/162 substrate; `lib/export/` module with types + tree-walker + validate + progress + storage + profiles + runner; size-limit validator with EPUB-as-fallback messaging; 8 hooks/route surfaces; phase7a-validate test suite 9 cases). 7.B JSON (pure-JSON renderer with backup format version "1.0"; includes documents + nodes + node_versions + context_nodes_referenced + context_links + node_comments + node_author_locks + layer_stack; emits stelavox_backup wrapper; phase7b-json-renderer test suite 6 cases). 7.C Outline (Markdown renderer with own tree-walk for depth/status/word_count_target; max_depth + include_word_count_target + include_status toggles; phase7c-outline-renderer test suite 7 cases). 7.D Renderers + UI (DOCX renderer via `docx` npm — letter/a4/6x9/mass_market page sizes, manuscript/kdp_paperback margins, configurable font + line spacing, 1440 twips per inch; EPUB renderer via `epub-gen-memory` — chapter-atomic structure, escapeHtml safety, configurable typography; ExportProgressChip 280px bottom-right + ExportProgressStack global; ExportModal format tile grid + profile selector with derived-effective-profileId render pattern; ExportHistoryPanel + Download/Re-run/Retry; DocumentExportButton with Cmd+Shift+E shortcut; AppShell mounts ExportProgressStack next to AppShellStatusIndicator; DocumentClient renders DocumentExportButton above NodeTree; phase7d-docx-renderer + phase7d-epub-renderer test suites 4+4 cases). **D2 Option B atomization**: resumable per-chapter Plan→Render→Assemble→Upload→Finalize pipeline; `runExportJob` orchestrator with cancellation check at chapter boundaries; renderers expose per-chapter progress callbacks; assembly-stage ceiling at ~5,000 pages DOCX / ~15,000 pages EPUB. **D3 Realtime DB subscription** for progress (export_jobs row subscribed via Supabase channel; no SSE). **D9**: JSON V1 is pure-JSON export-only (no zip, no attachments; import is V2). **D11**: skip Acts/Books as headings — Chapter = Heading 1 only (HEADING_NODE_TYPES = ['chapter']). **D12**: 4 built-in profiles — DOCX-Manuscript (Times New Roman 12pt, double-spaced, 1in margins, letter), DOCX-KDP (Garamond 11pt, single-spaced, 0.75in margins, 6x9 trim), EPUB-Standard (Lora typography), Outline-Structural (depth-3 + word counts). **OA-2 LibreOffice-on-Vercel was originally flagged as a 7.3 gating hazard but never materialised** — DOCX shipped via the `docx` npm package (pure-JS XML emit + zip), no headless office binary needed. §5 H-O-2 stays as a clarification: OA-2 applies to PDF (V2), not DOCX. **§5 Hazards** — no new hazards. Export operations are read-only against document state (no agent calls, no LLM, no scheduler-dependent execution); Storage interactions use Supabase Storage RLS; runner follows the established Option-B-like model (validated in V1.x-B.2 scheduler); all SECURITY DEFINER bodies include `SET search_path = public` per H-13. **Tier-A bumps in lockstep:** Component Spec v2.16 → **v2.17** (Export* component family); Product Spec v1.15 → **v1.16** (§4.13 Export System updated — DOCX + EPUB + JSON + Outline V1 with profiles; PDF/Markdown/KDP-as-separate stays Backlog; §4.14 cloud backup stays Backlog); CLAUDE.md v1.37 → **v1.38**. **Zero new verdigris uses** — Export modal primary action, status chip Download/Retry, profile-editor Save all use `--color-text-primary` neutral primary token; Cancel + Delete use destructive token; verdigris-use count remains nine. Migration count 157 → 162. **Reassigned to Phase 8 or Backlog:** JSON import (V2 — Phase 8 candidate if pulled in during Phase 9 review); PDF export via headless LibreOffice OA-2 path (Backlog — V2); KDP submission flow integration (Backlog — V2/V3); per-export attachment bundling (Backlog).
...
### H-26 — Derived-field staleness

**What happens:** A column is added to a table to hold a *derived* value (computed from another column) but no mechanism — trigger, RPC, application-side discipline, or background job — actually maintains it. The column ships populated by seed scripts at insert time, then every subsequent update to the primary column leaves the derived column NULL or stale. The bug is invisible if no UI surfaces the column live; it surfaces only when a programmatic consumer (an agent, an export, an analytics query) starts reading the value and reporting it.

**Why:** "Column with no maintenance contract" is a specification gap, not an implementation gap. The migration author defined the column intending it to be computed *somewhere* but the spec didn't say where. Subsequent write paths (PATCH, RPCs, agent-Accept) each had their own scope and none asserted ownership of the derived field. Tests typically assert *what the field accepts*, not *what it represents* — and per-function tests can't catch invariant violations between two fields.

**The fix:** Three parts.

1. **For new derived fields:** the migration that defines the column must also install the maintenance mechanism in the same change. Preferred: a `BEFORE INSERT OR UPDATE` Postgres trigger that recomputes the derived value from the source column whenever the source changes. This covers every write path (RPC, PATCH, direct UPDATE, future import paths) automatically without per-write discipline.

2. **For existing columns suspected of staleness:** audit by writing a single SQL query that compares the derived column against what it *should* be (e.g. `SELECT id FROM nodes WHERE word_count_actual IS DISTINCT FROM tiptap_word_count(prose)`). Backfill, then install the trigger.

3. **Test discipline:** every derived field gets layer-3 invariant tests asserting the contract *"after operation X on primary column Y, derived column Z is consistent."* One test per write path. Methodology in `memory/feedback_testing_methodology.md`.

**The fix is repeatable** — there is likely more than one instance of this pattern in any non-trivial schema. The first instance (`nodes.word_count_actual`) was caught 2026-05-18 via Director testing and remediated in M-172. A subsequent audit at the same time confirmed two sibling invariants (`node_versions` count ↔ `nodes.version`, and `briefs.current_stage_id` ↔ `brief_stages.status`) currently hold; no remediation needed there but both are now under test as layer-3 invariants going forward.

**Related hazards:** H-06 (Tiptap text extraction) — when a derived-field trigger is implemented in PL/pgSQL it can drift from a TS-side implementation of the same algorithm. Mitigate with a cross-language parity test.

**Scope:** Every column whose value is *derived* from another column or relationship in the same database. Candidates worth checking on top of those audited: any column with `_actual`, `_count`, `_total`, `_at` (when computed from peers) in its name; any column documented as "computed" in TA §3.6.
...
### H-28 — Version-bump trigger short-circuits on identical content

**What happens:** A trigger function gates a write-tracking action (in our case, bumping `nodes.version` and inserting a node_versions snapshot of meaningful content) on `NEW IS DISTINCT FROM OLD` AND on a caller-supplied intent signal (the GUC `stelavox.bump_version='true'`). When the caller's content happens to be byte-identical to the row's current content — which is rare but real for refines that re-emit the same prose, deterministic synthesise re-runs, or autosaves that wrote the same bytes between dispatch and accept — the trigger evaluates "no content change" and silently does nothing, even though the caller explicitly signalled intent. Per-call writes-without-bump accumulate: an accept-side INSERT into the snapshot table that runs unconditionally then produces multiple rows at the same un-incremented version number, breaking the implicit "(node_id, version) is unique" invariant downstream callers depend on.

**Why:** The trigger is implementing two intent models simultaneously — *autosave* (bump only on real change) and *agent accept* (always bump). Pre-fix the two-clause AND gate (`GUC AND content_changed`) meant the byte-equality check shadowed the explicit caller signal. The caller's GUC is supposed to be the dispositive signal for "this is a checkpoint event"; the byte diff is irrelevant for that intent — it's a relic from the autosave model where the byte diff *is* the signal. Mixing the two models in one condition silently sacrifices the agent-accept intent whenever the corner case lines up.

**The fix:** Three parts.

1. **GUC-first in the trigger.** When the caller sets the intent GUC, bump regardless of content delta. The trigger evaluates the *autosave* branch (byte diff) only when the GUC is unset. M-210 applies this to `bump_node_version_on_content_change`.

2. **Skip the snapshot when it carries no signal.** If an accept path doesn't change parent content (in our case, `accept_agent_job` for `operation_type='expand'` — children change but the parent doesn't), don't insert a snapshot row at all. A snapshot row of "what was identical to what's still there" is duplicate-version noise. M-210 makes the snapshot conditional on `operation_type <> 'expand'`.

3. **Structural backstop.** `UNIQUE (node_id, version)` on the snapshot table fails loudly the next time either root cause regresses. Pre-existing duplicate rows are renumbered chronologically before the constraint lands.

**Related hazards:** H-13 (SECURITY DEFINER `SET search_path`) — applies here; the trigger function declares `SET search_path = public`. H-26 (Derived-field staleness) — adjacent concept; both are "field not maintained the way its consumers assume." H-26 is about *missing* maintenance; H-28 is about *short-circuited* maintenance. The general principle for both: when the value carries a contract, the contract belongs at the maintenance site and a backstop belongs at the read site.

**Scope:** Every trigger or RPC that gates a write-tracking side-effect on `NEW IS DISTINCT FROM OLD`. Audit for callers that supply explicit intent (a GUC, a parameter, a status enum) where the byte diff is silently dispositive. Likely candidates: any future trigger that emits audit-log events, any trigger that maintains aggregate counts, any RPC that decides "is this a versioned operation?" based on content comparison.
...
| **6** | **Locking and workflow:** Author Lock UI + status simplification + unified write-gate + version restore. **MET** — Phase 6 shipped 2026-05-17 (Test Report v1.0). Three sub-phases on `claude/lucid-kare-fc8032` merged to master via `--no-ff`. 6.A Foundation (status enum 4→2; `check_node_writable` RPC; per-node lock semantics replace ancestor cascade; NodeLockIndicator extracted with V1.x-B.2-aware queue_status predicate). 6.B Author Lock (`node_author_locks` table + 6 SECURITY DEFINER RPCs + Lock/Unlock affordance + LockReasonModal + LockConflictModal + admin force-unlock with audit_log; per-org member RLS). 6.C Restore (`restore_node_version` RPC + VersionHistory Restore button on hover with disabled-state tooltips + RestoreConfirmModal with destructive token + "restored from v{N}" rendering + 409 conflict UX). 9 migrations (149-157); 20 new Vitest cases (9 6.A + 7 6.B + 4 6.C); 458/458 PASS. **D-A clarification**: comments + context links EXCLUDED from lock domain (annotation operations, not content). **D2**: locks are per-node with no implicit cascade; bulk-lock creates N independent rows with shared bulk_operation_id. **D7**: status reduces to {draft, approved} only. **D9**: Restore button uses destructive token (NOT verdigris) + confirmation modal teaches additive-history model. **D11**: every write endpoint routes through `check_node_writable`; F-190 round-3 audit ancestor-walk duplication closed. **Zero new verdigris uses**; Inviolable #2 count remains 9. **No new hazards.** | post-V1.x-F | **MET** — Lock and Progress core principle realised; status enum cleanup; restore action lands; write-gate consolidated. Migration count 148 → 157. |
| **7** | **Export:** DOCX + EPUB + JSON + Outline export with per-project profiles + resumable per-chapter pipeline. **MET** — Phase 7 shipped 2026-05-17 (Test Report v1.0). Four sub-phases on `claude/lucid-kare-fc8032` merged to master via `--no-ff`. 7.A Foundation (export_jobs 7-state pipeline + export_profiles + lib/export substrate + Storage upload + size validator with EPUB-as-fallback). 7.B JSON (pure-JSON renderer with backup format v1.0). 7.C Outline (Markdown renderer with depth/status/word-count toggles). 7.D Renderers + UI (DOCX via `docx` npm; EPUB via `epub-gen-memory`; ExportProgressChip + ExportModal + ExportHistoryPanel + DocumentExportButton with Cmd+Shift+E). 5 migrations (158-162); 30 new Vitest cases (9 7.A + 6 7.B + 7 7.C + 4+4 7.D); 484/488 PASS. **D2 Option B atomization**: resumable per-chapter Plan→Render→Assemble→Upload→Finalize; cancellation check at chapter boundaries. **D3 Realtime DB subscription** for progress (no SSE). **D11 HEADING_NODE_TYPES = ['chapter']** — Acts/Books skipped as headings. **D12** 4 built-in profiles. **OA-2 LibreOffice-on-Vercel never gated DOCX** — `docx` npm package is pure-JS (no headless office binary needed); OA-2 applies to PDF (V2). **Zero new verdigris uses**; Inviolable #2 count remains 9. **No new hazards.** | post-Phase-6 | **MET** — Author exports completed document to DOCX (manuscript + KDP profiles) / EPUB (Kindle-ready) / JSON (backup) / Outline (structural review); signed-URL retention 168h; resumable per-chapter pipeline; profile system per project. Migration count 157 → 162. |
| **8** | **MET 2026-06-09 — Polish and V1 release:** command palette (8.1), empty states (8.2), onboarding (8.3), keyboard shortcuts (8.4), performance review (8.5 + 8.5b + 8.5c), prose-editor three-dot menu (8.8), Sentence Focus end-to-end (8.9), Typewriter opt-in (8.10), `prefers-reduced-motion` audit (8.11), deferred Phase 3 motion tests TC-M-04 + TC-M-06 + TC-M-07 (8.12 — shipped 2026-06-09 in `tests/ui/sentence-focus-and-reduced-motion.spec.ts`). **Two original sub-items absorbed into later phases**: 8.6 V1 launch E2E suite → folded into Phase 10 (pre-launch detailed test) — same scope, same setup cost paid once; 8.7 production deploy hardening → folded into Phase 11 (operational hardening) — error monitoring + alert thresholds belong with DR + runbooks, and "tuned against real V1 traffic" needs Phase 10's load to calibrate. PR-gate CI/CD (lint + type-check + test must pass before merge) tracked as Backlog (separately addressable, low effort). 10 sub-items shipped 8.1, 8.2, 8.3, 8.4, 8.5, 8.8, 8.9, 8.10, 8.11, 8.12; 2 absorbed 8.6 + 8.7. **Sub-row 8.01 — UX Consistency (wireframe pass) MET 2026-05-31.** Eight wireframes locked under `docs/wireframes/wireframe_phase8_01_ux_consistency/` (Dashboard iter4, Edit Mode iter3, Project Page iter1, Detail Panes iter1, Director Mode iter1, Focus Mode iter1, iPad Layout iter1, Dashboard iter4 final). Brand Identity bumped v2.0 → v2.2 — **new Inviolable #6 — Cormorant Garamond italic appears only in the wordmark** (closes the asymmetry against Inviolable #3 Cinzel-only); lozenge implementation locked as 45°-rotated square with 8px verdigris glow. Component Spec bumped v2.20 → v2.21 — new §18 "Phase 8.01 — UX Consistency" documents the bracketed monospace label vocabulary (`[Book 1]` / `[Act 1]` / `[Ch 1]` / `[Sc 1]` / `[Bt 1]` — Option A locked, position number from `nodes.order`), the canonical responsive-breakpoint table (≥1280 three-pane / 1024–1279 two-pane / 768–1023 single-pane with slide-overs), touch interactions table, Dashboard component family (Wordmark + Sidebar + Resume Writing hero + Needs Attention strip + ProjectGrid populated shape; EmptyHero + EmptyActions + 3-tile philosophy strip first-time shape), QuickStartChecklist (5 items, progressive tick + condense-and-remove), YoursTile with `ENCRYPTED AT REST · PRIVATE TO YOUR ACCOUNT` lock line, SampleNovelImportModal with `SAMPLE` badge convention, DirectorPanel rendering refinements (`<plan>` → collapsed "Reasoning · N lines" chip; tool-call chips; inline workflow-proposal cards with verdigris border; `@` mention picker with positional path `@act1ch1sc1bt2`), Focus Mode breadcrumb opacity bumped 0.2 → 0.35 steady / 0.7 hover, FocusEscHint repositions bottom-left and gains touch "← Edit" pill variant, NodeRow min-height 36/44 split → 44 universal. Implementation work (the actual UI build against these wireframes) follows the remaining 8.x sub-items. **No verdigris-use count change** (remains nine); two open Inviolable audit items raised (YoursTile decorative dot; mentioned-node tree-row border — both within existing categories). | post-Phase-7 | V1 launch-grade UI + perf + ops ready |
| **9** | **Pre-launch backlog review — 9.A–9.D MET 2026-06-10.** Executed as a full project reset: every deferred / queued / V2-V3-tagged item across all sources (Director Architecture §16.3 + §16.4, this doc's Backlog row, per-phase memos, Round-3 audit residual at theme level, open SUs, CLAUDE.md changelog sweep) inventoried into the **V1 Deliverables Register** — `stelavox_v1_deliverables_register_v1_0.md`, a Tier-A planning artefact that is now the single source of truth for undone work; items not in the register don't exist. **121 items (DR-001..DR-121) across 11 categories, ALL bucket-assigned by the author in one review session:** 22 V1 · ~42 V1.x · ~40 V2+ · 2 V3 (soft-drop) · 2 dropped (DR-044 Brief-aware lock conflict + DR-053 BriefViewer concurrent indicator — both obsoleted by the Director simplification). The 22 V1 items organise into **five work packages**: A cloud cutover (scheduler-listener cloud home + cloud DB sync + cron probes) · B Stripe · C security + correctness hardening (9 audit-derived items) · D Director context-node create + link · E UX finish-the-job (5 items) — plus 3 items folded into Phase 10 (export size banners, Inviolable audit close, Day-1 observability verification). **Product-scope locks made during the review:** V1 plan lineup = free trial month + single-user platform plans (writer / author / pro) + single-user BYOK (`byok_solo`; `byok_team` stays dormant data until multi-user orgs in V2); Stripe is V1 and every plan including BYOK pays a platform fee; credit exhaustion locks Director/agent dispatch only — never writing (escape: upgrade or BYOK-convert); V1 = single-user orgs only; NO injection-scanner override ever (clear security-warning UX instead); rate-limit lookups fail closed; subscription management via Stripe hosted Customer Portal. **9.A–9.E MET 2026-06-11.** 9.E sequencing locked: C first → A before any production deploy → B + D parallel-capable → E anytime. | post-Phase-8 | ✅ Register locked, propagated, all five work packages sequenced |
| **9.1** | **Security + correctness hardening (work package C) — SHIPPED 2026-06-11.** 9 V1 register items closed: DR-095 (rate-limit fail-closed) · DR-096 (audit_log critical-event writes + admin-section viewer per wireframe_admin_audit_log_v1) · DR-097 (silent-failure ESLint `no-empty` guard; recon confirmed all hot-path sites pre-closed) · DR-098 (`.single()` lib/data sweep — recon confirmed pre-closed) · DR-100 (M-215 `increment_usage_record` atomic UPSERT closes the F-133/F-134 billing-row-loss race + on-failure audit_log write) · DR-102 (F-89 fix — `assertConversationAuthor` now falls back to org-membership instead of admitting any caller) · DR-103 (injection frame — recon confirmed all 5 sites pre-closed) · DR-111 (`getConfig` runtime validation — recon confirmed pre-closed; 61 typed call sites) · DR-112 (`decorateWithLeaf` — recon confirmed pre-closed). 8/8 new Vitest cases PASS; full suite 1073/1073 PASS; v1x-apollo 211+1baseline-flaky. Migration count +1 (M-215). **Recon-shrink**: budgeted 9-item build became 4-build + 5-verify + 1 wireframe-driven UI half; recon was load-bearing. | post-Phase-9 | ✅ 9 items closed; see Test Report v1.0 |
| **9.2** | **Cloud cutover (work package A) — SHIPPED 2026-06-11.** 3 V1 register items closed: DR-063 (Director Architecture §15 — Option E pg_net transport in cloud; local LISTEN listener preserved at ms-latency; CAS-on-claim makes double-fire safe) · DR-067 (stelavox-dev reset + 208 migrations applied — and surfaced the 7-file date-stamp/sequence-number inversion bug whose pure-rename fix protects every future env, prod included) · DR-117 (synthetic probes ride the same pg_net transport — no Vercel-Cron slots consumed). Migration count +2 (M-216 transport + M-217 timeout hotfix). Cloud activation proven: `dispatcher-tick` accepts CRON_SECRET → 200; manual + cron-driven invocations recorded 200 OK in `net._http_response` with no error_msg; `dispatcher_sweep_http` 1-minute pg_cron job firing autonomously. **Enqueue-trigger end-to-end deferred to first real cloud signup** (post-reset DB has zero organisations; hand-crafting a test fixture would pollute the clean DB; the trigger condition is structurally verified by the Vitest suite locally). | post-Phase-9.1 | ✅ 3 items closed; transport autonomous; see Test Report v1.0 |
| **9.B** | **Stripe integration (work package B) — SHIPPED 2026-06-11.** 1 V1 register item closed: DR-070 (Stripe rails for collect-payment / subscriptions / webhooks / dunning). **3 migrations** (M-219 `stripe.*` platform_config keys · M-220 `organisations.{trial_expires_at, stripe_price_id}` + revised `handle_new_user` · M-221 `subscription_events.stripe_event_id` UNIQUE partial index). `stripe@22.2.0` npm package; `lib/stripe/{config,client,customers,sessions,plans,webhook-handlers}.ts`; `lib/billing/trialExpiry.ts`. **3 API routes**: POST `/api/billing/checkout` (validates plan ∈ STRIPE_PLAN_SLUGS, find-or-creates Customer, creates Checkout Session) · POST `/api/billing/portal` (requires `stripe_customer_id`, creates Portal session) · POST `/api/stripe/webhook` (signature verification + 11 event types + idempotent INSERT + per-event handlers). **Trial-expiry gate** in `app/(app)/layout.tsx`: trial orgs whose `trial_expires_at` has elapsed get redirected to `/settings/plan?reason=trial_expired`; exempt paths are `/settings/plan*`, `/api/billing/*`, `/api/stripe/*`. Per the product lock — "credit exhaustion never blocks writing" applies to paid plans only; trial users hit the expiry redirect when time runs out. **UI**: SubscribeButton + ManageSubscriptionButton mounted in PlanPanel; `/settings/plan` shows status banners on `?stripe_status=success`/`cancelled`/`?reason=trial_expired`. **Build against Stripe test mode** by default (`stripe.mode` config key); swap to live via DB UPDATE — no deploy. Substrate-only state ships safely (503 `stripe_not_configured` with `missing` list until account provisioned); setup checklist in `stelavox_phase9_b_session1_test_report_v1_0.md` §6. Migration count moves 218 → 221. **32 new Vitest** (stripe-config 10 + trial-expiry 12 + stripe-plans 6 + stripe-webhook-handlers 4) PASS. Apply-time discovery: `subscription_status` CHECK uses British spelling (`cancelled`, `trialling`) while Stripe uses American (`canceled`, `trialing`); `normaliseSubscriptionStatus` handles the mapping. End-to-end activation gated by user-driven Stripe account setup. | post-Phase-9.D | ✅ DR-070 closed; substrate ready; Stripe account provisioning is the gate |
| **9.D** | **Director context-node create + link (work package D) — SHIPPED 2026-06-11.** 1 V1 register item closed: DR-051 (Director auto-create + auto-link of context nodes when `generate_context` targets a structural node). **Substrate already shipped** — executor at `lib/director/workflow-executor.ts:496-578` has been live since 2026-05-08 (Mars-series cloud finding closed under SU-J11-2 Option B); DR-051 was a prompt + documentation gap, not a substrate gap. **M-218 publishes Director config v1.29** with the prompt rewritten in two passages (Step shapes line + Trust-the-specialists block) to teach the model the auto-create + `structural_to_context` link path. Tool registry unchanged at 17 (12 read + 5 write). **Discovery during apply:** v1.25/v1.26/v1.27/v1.28 had each shipped without changelog entries — back-documented in this row. Migration count +1 (M-218). **Verification:** 3 integration cases in `workflow-executor-context.test.ts` (auto-create + link + re-target against real local DB) + 1 drift-guard case in `director-prompt-vs-schema-drift.test.ts` (pins v1.29 framing, asserts pre-v1.29 paragraph removed). H-08 propose-only preserved (executor's auto-create runs at dispatch time, not inside the Director's tool-use loop). | post-Phase-9.2 | ✅ DR-051 closed; v1.29 production; SU-J11-2 memo closed |
| **10** | **Pre-launch detailed test.** Comprehensive cross-surface test pass: every author journey (signup → project → document → write → Director → export); every Director scenario (single-stage / multi-stage Brief / amendment / cancellation / capability-limit self-rejection / Stop+Resume); every admin operation (dashboard / probe trigger / capacity alerts); every failure-class UX. Includes load-shape tests at expected V1 traffic. **Absorbs original 8.6 V1 launch E2E suite** — same scope, same fixtures; consolidated 2026-06-09 to avoid paying the setup cost twice. | post-Phase-9 | Every V1 feature verified end-to-end against the cross-surface test matrix; perf + failure modes hold at expected load |
| **11** | **Pre-launch operational hardening.** Disaster recovery (backup + restore procedures verified end-to-end with a full DB restore drill; data export for users; document-level restore from versions); incident runbooks (rate-limit response, Anthropic outage, DB unavailability, Vault key rotation); monitoring + alerting baselines (capacity-alert thresholds tuned against real V1 traffic; on-call paging surface); on-call coverage plan; security-incident response procedure. **Absorbs original 8.7 production deploy hardening** — error monitoring wiring + production-readiness checks share infrastructure with monitoring + alerting baselines + runbooks; consolidated 2026-06-09. Production-deploy-pipeline hardening (manual approval gate, smoke test before rollout, rollback affordance) lives here too. PR-merge-gate CI/CD (lint + type-check + test must pass) is Backlog — separately addressable and low effort. | post-Phase-10 | DR drill passed; runbooks signed-off; on-call paging functional; monitoring + alerting calibrated against real load |
...
| **14** | **Layer Stack Generalisation — post-V1.** Architectural model locked at Phase 8.01 (`memory/project_layer_stack_generalisation.md` is the durable record). V1 launches with two layer_stacks (Novel + Series-of-Novels) hardcoded in `components/tree/NodeTree.tsx`'s `LAYER_LABELS` / `NODE_TYPES` maps; this phase makes the consumers read from the layer_stack data. **Six work items** (sizes roughly ascending): (14.1) drop or replace the `nodes.node_type` CHECK constraint with layer_stack-aware write-time validation via a constraint trigger; (14.2) extend the `layers` JSONB shape with display + reference fields (`abbreviation`, `singular_label`, `plural_label`, `at_reference_token`, `default_word_count_target`, `agent_profile_mapping`); (14.3) add a GET endpoint for layer_stacks (`/api/projects/[id]/layer-stack`); (14.4) rewrite `NodeTree.tsx` + every other frontend site that hardcodes node-type assumptions to consume the fetched layer_stack — the component becomes truly polymorphic over document type; (14.5) move `layer_stack_id` from `documents` to `projects` (small migration, one stack per project, immutable after creation); (14.6) separate Director config per layer_stack type (`director_configs.layer_stack_id NULL` = default; resolver at turn start) — plus authoring new Director system prompts and per-node-type agent_profiles for any new stacks. Plus data-only work (no code): admin-defined new layer_stacks (Anthology, Paper, TV Series, Non-fiction Series, Screenplay) authored as demand emerges. **Cross-cutting consumers that change in 14:** NodeTree display, Director `@`-reference syntax (abbreviation token per layer becomes `act1ch2sc3bt1`-style derived from layer_stack), agent profiles (one set per node_type), Director system prompts (one config per layer_stack), `create_document_with_layer_stack` RPC, export (tree-with-checkboxes works with any layer_stack), project creation flow (user picks layer_stack at creation; immutable thereafter). **Out of scope for Phase 14:** user-editable layer_stacks (admin-only), cross-project series-sharing (V2+), bound-trilogy multi-document export (separate feature), mixing doc types within one project (explicitly prohibited by the model). | post-V1-launch | Six work items shipped; admin can add new layer_stacks via data-only work; all frontend consumers polymorphic over the fetched layer_stack |
| **Backlog** | **Superseded as an inline inventory by the V1 Deliverables Register (Phase 9, 2026-06-10).** `stelavox_v1_deliverables_register_v1_0.md` is the single canonical list of all undone work — every item from the old inline inventory here (Director Architecture §16.3, this doc's V2 list, per-V1.x deferrals, Phase 3a document operations, developer documentation) is captured there with a DR number and an author-locked bucket (V1 / V1.x / V2+ / V3 / dropped). Post-launch prioritisation works from the register's V1.x and V2+ buckets. Notable bucket moves vs the old inline list: Stripe → **V1**; invitation flow → V2+ (single-user-org V1 lock); audit-log admin viewer → V1 (DR-096); FailureToast/Banner adoption → V1 (DR-020). | ongoing | Register is the source of truth; continuously prioritised post-V1 launch |

**Phase ordering note:** the V1.x-LB through V1.x-F rows shipped chronologically between the original Phase 5d and Phase 6 ship-dates (2026-05-12 to 2026-05-19). They are *additional* substrate (Director V2 / scheduler / Brief / admin / failure-mode UX) inserted after the Director V2 deep-dive deemed them required for V1; they did NOT replace Phase 6 / 7 / 8 which remain mandatory pre-launch. The original 7-row plan locked at Director V2 deep-dive (V1.x-LB → V1.x-F) is complete as of 2026-05-19 (`v1.x-f` tag); Phase 6 + 7 + 8 are MET. Phase 9 + 10 + 11 + 12 + 13 are next.
...
**v2.17 — 2026-05-31** **Phase 8.01 close-out — UX Consistency wireframe lock + Phase 14 added to plan.** §11 Phase 8 row now carries a Sub-row 8.01 MET note: eight wireframes locked under `docs/wireframes/wireframe_phase8_01_ux_consistency/` (Dashboard iter4, Edit Mode iter3, Project Page iter1, Detail Panes iter1, Director Mode iter1, Focus Mode iter1, iPad Layout iter1, Dashboard iter4 final). Brand Identity bumped v2.0 → v2.2 — **new Inviolable #6 — Cormorant Garamond italic appears only in the wordmark** (closes the asymmetry against Inviolable #3 Cinzel-only; was caught by the Dashboard iter4 silent regression that collapsed the mark to a single Cinzel "Stelavox"). Lozenge implementation locked as a `transform: rotate(45deg)` square (NOT a primitive diamond glyph) with 8px verdigris glow `box-shadow: 0 0 8px rgba(90,168,122,0.45)`. Component Spec bumped v2.20 → v2.21 — new §18 "Phase 8.01 — UX Consistency" documents the bracketed monospace label vocabulary (`[Book 1]` / `[Act 1]` / `[Ch 1]` / `[Sc 1]` / `[Bt 1]` — Option A locked, position number from `nodes.order`); the canonical responsive-breakpoint table (≥1280 three-pane / 1024–1279 two-pane / 768–1023 single-pane with slide-overs / <768 backlog); touch interactions table (long-press 350ms drag, 800ms context menu — tighter than iOS standard); Dashboard component family (Wordmark + Sidebar three-section + Resume Writing hero + Needs Attention strip + ProjectGrid populated shape; EmptyHero + EmptyActions + 3-tile philosophy strip first-time shape); QuickStartChecklist (5 items, progressive tick + condense-and-remove); YoursTile with `ENCRYPTED AT REST · PRIVATE TO YOUR ACCOUNT` lock line; SampleNovelImportModal with `SAMPLE` badge convention; DirectorPanel rendering refinements (`<plan>` → collapsed "Reasoning · N lines" chip; tool-call chips; inline workflow-proposal cards with verdigris border; `@` mention picker positional-path syntax `@act1ch1sc1bt2`); Focus Mode breadcrumb opacity bumped 0.2 → 0.35 steady / 0.7 hover (revising the previously-locked §6.2 ceiling); FocusEscHint moves bottom-left and gains touch "← Edit" pill variant; NodeRow min-height 36/44 split → 44 universal. Cross-cutting consumer count (component sections touched by this consolidation): 11 existing + 8 new in §18. **No verdigris-use count change** (remains nine); two open Inviolable audit items raised (YoursTile decorative dot; mentioned-node tree-row left border — both within existing categories pending audit). **NEW §11 row — Phase 14 — Layer Stack Generalisation (post-V1)** added between Phase 13 and Backlog: six work items captured in the row plus the durable record at `memory/project_layer_stack_generalisation.md`. V1 launches with two layer_stacks (Novel + Series-of-Novels) hardcoded in `components/tree/NodeTree.tsx`; Phase 14 makes the consumers polymorphic over the fetched layer_stack data. Architectural model decided at Phase 8.01: Project = workspace defined by layer_stack; Series is not special, it's one node_type among others in some stacks; the "series concept" lives at Project level when sub-units are large (novel series — own documents) and as a tree layer when sub-units are small (anthology, TV series — share one document); one layer_stack per project; admin-managed; each layer_stack gets its own Director profile. **Implementation work for the wireframes themselves (Phase 8.01 build pass against the locked wireframes)** continues as part of Phase 8 — the wireframe lock is the spec contract; the build pass turns wireframes into code. Phase 8 remains in flight; no other phase moves. No new hazards; no migrations in this consolidation; no schema change.
