...
> **Director simplification — one approve per brief, no interim approvals (2026-05-21):** **User-visible:** (1) **One active Brief per document.** Starting a new Brief while one's running queues no longer exists; the user is asked to cancel or finish the active one before starting another. Reverses V1.x-B.3's multi-active feature, which had no real use scenarios. (2) **Auto-approve flag covers the WHOLE Brief.** If the user checks "Auto-approve subsequent stages" when approving a multi-stage Brief, every stage's workflow runs without further clicks — including stages whose workflows are planned just-in-time when their trigger fires. The previous half-measure (auto-approve covered the initial workflow but not subsequent amendments) is gone. One flag, one meaning. (3) **BriefProposalCard now shows each stage as either pre-planned steps OR a prompt** describing what the system will plan when that stage's turn comes. (4) **No more BriefAmendmentCard.** If the user wants to change a Brief mid-flight, the path is "cancel the current Brief, propose a new one" — simpler, more predictable, no edge cases. **Architect rationale:** Four schema-vs-code drift bugs in 48 hours of testing the V1.x-B.3 amendment surface motivated removing the entire mechanism. The new model handles the dominant use case (multi-stage Brief where stage N's targets depend on stage N-1's outputs) via a system-driven planning loop the LLM doesn't have to know about — the LLM just describes stages with prompts; the system invokes the LLM with each prompt when its trigger fires; the workflow gets attached automatically. Result: same outcome, smaller surface area, fewer failure modes.
...
The user provides their own LLM provider API key (Anthropic or OpenAI at launch). They pay their API provider directly for every token consumed. Stelavox charges a flat fee for platform access only — **collected through Stripe like every other plan** (locked at the Phase 9 review, 2026-06-10: every plan including BYOK pays a platform cost). No token quotas are enforced by Stelavox — the user's own provider rate limits apply. BYOK users receive full feature access including the Director.

> **V1 scope (locked 2026-06-10, Phase 9 review):** V1 ships **single-user organisations only** — the V1 plan lineup is the 30-day Trial, the three single-user Platform tiers (Writer / Author / Pro), and **BYOK Solo**. **BYOK Team is not offered in V1**: it is a multi-user plan and follows multi-user organisations + the invitation flow to V2. The `byok_team` plan row remains in the database as dormant data with no Stripe product and no signup path.
...
**The writing surface is never locked (locked 2026-06-10, Phase 9 review).** Budget exhaustion pauses Director and agent dispatch only — the author can always continue writing, editing, organising, and exporting their work regardless of credit state. The two escape paths from exhaustion are: **upgrade to the next Platform tier**, or **convert to a BYOK plan** (both self-serve via Stripe). Mid-period top-ups are deliberately out of V1 scope (V2+ — Deliverables Register DR-015); the upgrade/convert paths are the V1 answer.
...
**v1.19 — 2026-06-10** **Phase 9 review — V1 commercial scope locked.** §3.1 amended: V1 ships **single-user organisations only** — plan lineup is 30-day Trial + Writer/Author/Pro + **BYOK Solo**; **BYOK Team is not offered in V1** (follows multi-user orgs + invitations to V2; `byok_team` plan row stays as dormant data, no Stripe product, no signup path). BYOK platform fee explicitly collected through Stripe like every other plan. §3.2 amended with the **"writing surface is never locked" guarantee**: budget exhaustion pauses Director/agent dispatch only; writing, editing, organising, and exporting always work. Escape paths from exhaustion: upgrade tier or convert to BYOK (self-serve via Stripe); mid-period top-ups deliberately out of V1 (Deliverables Register DR-015 → V2+). All other decisions from the Phase 9 review live in the new Tier-A planning artefact `stelavox_v1_deliverables_register_v1_0.md` — 121 items bucket-assigned; the register supersedes scattered V2/V3 markers project-wide. §3.4 Stripe infrastructure already specified (Checkout + Webhooks + Customer Portal) — unchanged, now confirmed V1 (DR-070).

**v1.18 — 2026-05-31** Phase 8.01 — UX Consistency wireframe-lock absorption. Eight wireframes locked under `docs/wireframes/wireframe_phase8_01_ux_consistency/`. **No new product feature added; no existing surface removed.** This version captures user-facing language and presentation refinements only — the actual build pass against the wireframes happens within Phase 8. **Onboarding language locks:** the first-time tagline reads *"A hierarchical writing workspace where structure stays in your hands and AI helps where you ask it to."* The two first-time CTAs are "Get started" (verdigris primary → blank Novel project, no wizard) and "Try the sample novel" (ghost secondary → import a pre-built sample). The first-time philosophy strip presents three tiles — Structure-first writing · A Director, not an author · Your work, yours alone — with the third tile carrying a small lock line `ENCRYPTED AT REST · PRIVATE TO YOUR ACCOUNT` (accurate; doesn't overclaim end-to-end). The sample-novel surface is tagged `SAMPLE` in the author's library so it cannot be mistaken for their own work. **Quick Start checklist** in the first-time sidebar carries 5 items (Sign in / Create your first project / Add a beat and write / Try the Director / Export your first chapter), ticks progressively as each is completed, and condenses to "Setup complete ✓" for one session before disappearing — onboarding scaffolding, not a permanent fixture. **Dashboard populated-state language:** the "Resume Writing" hero shows the last beat the author worked on with bracketed-path crumb (`[Book 1] · [Act 1] · [Ch 1] · [Sc 1] · [Bt 3]`) and "Continue writing →" CTA. **Director Mode rendering:** `<plan>` blocks in Director responses become a single collapsed "Reasoning · N lines" chip (tap to expand); tool calls render as compact monospace chips (consecutive read-tools may group as "Looked at N nodes"); workflow proposals render inline as small cards with verdigris border and Approve/Modify (the detail-panel `PlanCard` remains the authoritative dispatch surface). The Director `@` mention picker accepts both partial-name search (`@iron`) and positional path (`@act1ch1sc1bt2`). **Focus Mode:** ⌘ ← / ⌘ → cycle prev/next sibling beat with wrap to next parent's first child (iPad: swipe left / right). The Focus Mode breadcrumb is more readable than before — at-rest opacity 0.35 (was 0.2) carries the bracketed-path vocabulary; hover/touch bumps to 0.7 to reveal the position counter "2 / 5" and node name. **Bracketed-label vocabulary** (`[Book 1]` / `[Act 1]` / `[Ch 1]` / `[Sc 1]` / `[Bt 1]`) is now the canonical visual language for layer positions across every surface that shows hierarchy (tree, detail-pane crumb, Focus breadcrumb, Director `@` picker, export tree-with-checkboxes, dashboard hero). For the Series-of-Novels layer_stack, the leading `[Series]` label is added. Position number is read from `nodes.order`. **iPad** is now a first-class viewport with a documented responsive contract — see Component Spec v2.21 §18.2. Five Inviolables become six: **Inviolable #6 — Cormorant Garamond italic appears only in the wordmark** added (Brand Identity v2.2; closes the asymmetry against Inviolable #3 Cinzel-only). Verdigris-use count remains nine. **Phase 14 — Layer Stack Generalisation** added to TA §11 as post-V1 work — V1 ships Novel + Series-of-Novels hardcoded; post-V1 makes consumers polymorphic over the fetched layer_stack so further document types (Anthology, Academic Paper, TV Series, etc.) can be added via data-only admin work. Pricing tiers (§3.1), Out of Scope (§7), and Locked Decisions (§8) unchanged.
