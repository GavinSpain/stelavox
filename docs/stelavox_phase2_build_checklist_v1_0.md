...
Authoritative spec: **Brand Identity v2.0 §8.2** (Edit Mode three-panel layout — Tier 0); `stelavox_component_specification_v2_0.md` §2 (Layout Components); `stelavox_ui_design_specification_v1_0.md` (tokens); `wireframes/wireframe_edit_mode_v1.html` (visual reference; read `wireframes/stelavox_wireframe_errata_v1_0.md` first per Session Start Checklist — the errata makes no layout corrections, so the wireframe's dimensions are canonical).

The Phase 1 `(app)/layout.tsx` is a placeholder. Phase 2 replaces it with the three-panel `AppShell` per Brand Identity §8.2:

- **Left panel — Sidebar (220px expanded, collapsible to 48px icon rail).** Project navigation, context library (Phase 4 content), settings footer.
- **Centre panel — NodeTree (`flex: 1`, `min-width: 320px`).** The primary map. Built via react-arborist in §3.4. §3.2 leaves this slot as an inline placeholder until T-4.x lands.
- **Right panel — DetailPanel (380px, resizable to 540px).** Node detail tabs (Content / Agent / Comments / History / Context), all built in Phase 3. §3.2 leaves this slot as an inline placeholder.

The shell is `display: flex; flex-direction: column`. Header (48px, `flex-shrink: 0`) on top. Body row (`display: flex; overflow: hidden; flex: 1`) underneath, composed of: Sidebar + PanelResizer + NodeTree slot + PanelResizer + DetailPanel slot. Background `--color-bg-base`.

Phase 2 implements only **Edit Mode**. The `Director` and `Focus` mode states arrive in Phase 5 and Phase 3 respectively — but the AppShell is structured so they slot in without architectural change (Sidebar collapse to icon rail, DetailPanel→DirectorPanel swap, full-screen Focus overlay above the shell).

> **Workflow note.** Build T-2.2 (Header), T-2.3 (Sidebar), T-2.4 (PanelResizer) first as fully-implemented components. Then assemble them in T-2.1 (AppShell). Then wire the shell into the route in T-2.5. The middle (NodeTree) and right (DetailPanel) slots are inline placeholder divs at correct dimensions until their respective build phases.

- [ ] **T-2.1.** Create `components/layout/AppShell.tsx`. Top-level shell: column flex layout; fixed 48px `<Header />` (`flex-shrink: 0`); horizontal flex body row beneath (`overflow: hidden; flex: 1`). The body row composes (left to right): `<Sidebar />` (220px), `<PanelResizer position="sidebar-tree" />`, NodeTree slot (inline placeholder `<div>`, `flex: 1`, `min-width: 320px`, neutral background, label "Document tree" in `--color-text-muted`), `<PanelResizer position="tree-detail" />`, DetailPanel slot (inline placeholder `<div>`, 380px, neutral background, label "Node detail" in `--color-text-muted`). Background `--color-bg-base`. Acceptance: shell renders against any authenticated route; the dashboard and the project page (Phase 1 surfaces) render correctly inside the shell; `npm run build` passes; visual proportions match `wireframes/wireframe_edit_mode_v1.html` at 1280px viewport width.

- [ ] **T-2.2.** Create `components/layout/Header.tsx`. 48px height, `--color-bg-surface` background, `1px solid --color-border-subtle` bottom border, padding `0 20px`, `z-index: 10`. Child order per Component Spec §2.2: `<Wordmark>` (`margin-right: 28px`), `<HeaderBreadcrumb>` (`flex: 1`), `<ModeTabBar>` (`margin: 0 20px`), `<HeaderActions>` (`margin-left: auto`). Phase 2 content: `<Wordmark>` is the placeholder text "Stelavox" (Inter 500 16px) — the brand `<Wordmark>` component is not in Phase 2 scope; `<HeaderBreadcrumb>` is an empty placeholder; `<ModeTabBar>` renders three tabs per Component Spec §2.5 (Edit active; Director and Focus rendered as disabled placeholders); `<HeaderActions>` shows user email + sign-out, carrying forward the Phase 1 stub. All values via CSS custom properties (no hardcoded hex per the Design Token Rules). Acceptance: Header renders; visual proportions match Component Spec §2.2 and the wireframe.

- [ ] **T-2.3.** Create `components/layout/Sidebar.tsx`. Width 220px expanded, 48px collapsed (icon rail). Background `--color-bg-surface`, border-right `1px solid --color-border-subtle`, width transition `--duration-normal --easing-smooth`. Three vertical sections per Component Spec §2.3: project navigation (top, fixed height); context library (middle, `flex: 1`, scrollable; collapsible per-type sections — Characters, Locations, Themes, etc.); settings footer (bottom). Phase 2 content: project nav shows the active project + document name (placeholder text from route params); context library renders its scaffold but is empty (Phase 4 populates); settings footer is a stub link. Collapse state persists in `localStorage` key `stelavox_sidebar_state`. **Important — separation of concerns:** the Sidebar does NOT contain the node tree. The node tree is the centre panel of `AppShell`, built in §3.4. Conflating them breaks Brand §8.3's Director Mode (Sidebar collapses to icon rail, tree remains visible) and §7.9's Focus Mode transition (tree and sidebar retract independently). Acceptance: Sidebar renders the three-section structure with placeholder content at 220px; collapse to 48px works; collapsed state persists across reloads.

- [ ] **T-2.4.** Create `components/layout/PanelResizer.tsx`. 4px wide vertical drag handle, `cursor: col-resize`. Colours: default transparent; hover `--color-border-default`; **active (dragging) `--color-border-strong`** — Inviolable #2 / Component Spec §2.4 / Wireframe Errata: this is NOT a verdigris use; `--color-accent` MUST NOT appear in this file. Width transition `--duration-fast` on colour. Constraints per Component Spec §2.4: Sidebar `[220, 340]` expanded; DetailPanel `[320, 540]`; NodeTree `min-width: 320` (flex constraint). Persists widths in `localStorage` keys `stelavox_sidebar_width`, `stelavox_detail_width`. Two instances are mounted in AppShell (`position="sidebar-tree"` and `position="tree-detail"`); the `position` prop selects which localStorage key the resizer reads/writes. Acceptance: dragging each handle updates the adjacent panel widths within bounds; CSS audit confirms `var(--color-accent)` does not appear in this file; both `localStorage` keys persist across reloads.

- [ ] **T-2.5.** Replace the Phase 1 stub `app/(app)/layout.tsx` with `<AppShell>` from T-2.1. Verify no Phase 1 surface (dashboard, project page, document page) regresses. Acceptance: TC-U-06 to TC-U-12 from **Phase 1's** test plan still pass against the new shell. `npm run build`, `npm run lint`, `npm run type-check` all pass.
...
**v1.1 — 2026-05-04** Corrected §3.2 (App shell — layout primitives). The original §3.2 specified a 280px sidebar with the NodeTree inside it and a 400px detail panel via CSS grid. This conflicted with **Brand Identity v2.0 §8.2** (Tier 0 — "three-panel layout: Left 220px · Centre tree flex · Right 380px"), **Component Specification v2.0 §2.1** (matching dimensions, flex layout), and `wireframes/wireframe_edit_mode_v1.html` (visual ground truth — `.sidebar 220px`, separate `.tree-panel`, `.detail-panel 380px`). The Wireframe Errata v1.0 makes no layout corrections, confirming the wireframe dimensions are canonical. Classification per global CLAUDE.md: **specification error** in the Build Checklist alone — Brand Identity, Component Spec, and the wireframe agreed. Beyond the dimensions, the Build Checklist's architecture would have broken Brand §8.3 (Director Mode requires the right panel to be a swappable slot distinct from the persistent centre tree), §8.4 (Edit→Director transition: detail panel dissolves, Director panel slides in from right), and §7.9 (Focus Mode: prose expands from the DetailPanel slot). Discovered before any §3.2 implementation began. §3.1 (migrations and types) is unaffected and was already shipped on `claude/phase2-foundation` prior to this audit.
