...
|---|---|---|---|---|---|
| §2 UI checkpoint (TC-U-01..13, U-15..24 from v1.0; U-25..28 from v1.1; U-29 from v1.3) | 28 | 22 | 6 | **28/28 PASS** | TC-U-14 |
...
| **Total** | **99** | **80** | **20** | **99/99 PASS** | **4 deferred** |
...
### Post-merge UX-test finding — Structural UI rendered in browser default, not Inter *(v1.6)*
- **Classification:** Implementation gap — Inviolable-adjacent. Brand Identity v2.0 §6 mandates Inter for all structural UI; Inviolable #4 makes the typeface boundary absolute. The implementation set `--font-sans: var(--font-sans)` in `app/globals.css` line 69 — a circular self-reference that resolves to nothing. The body's `font-family: var(--font-sans)` therefore fell through to the browser's native sans-serif default (Segoe UI on Windows, Helvetica Neue on Mac), and every UI surface that didn't explicitly opt into Inter inherited the browser default.
- **User-visible symptom:** the running app's tree rows, sidebar items, header text, tab strip, panel headers, and dialog text rendered in Segoe UI instead of Inter. The wireframe (`docs/wireframes/wireframe_edit_mode_v1.html`) loads Inter via Google Fonts and applies it to body, so the tested rendering looked visibly different from the wireframe spec — a font weight and letterforms drift the user spotted by eye.
- **Why it wasn't caught:** Phase 3's typeface tests (TC-U-02 SummaryEditor, TC-U-03 ProseEditor) only verified the *editor* surfaces, where I had explicitly applied `var(--font-inter)` via `data-editor` selectors. The structural surface had no test, and the editor tests passed because they never read the body or any non-editor element.
- **Root cause:** the original token declaration was inherited from create-next-app's Geist boilerplate. When Phase 3 added Inter for the editor surfaces, the body-level `--font-sans` was never wired up to the new Inter variable. The token line was a self-reference, which compiles cleanly but resolves to the empty string at runtime.
- **No spec amendment.** Brand Identity v2.0 §6, Component Spec v2.5, and Inviolable #4 are all already correct — they always required Inter. Only the implementation diverged.
- **Fix:** `app/globals.css` line 69 changed from `--font-sans: var(--font-sans)` to `--font-sans: var(--font-inter)`. The Inter variable is already declared on `<html>` via `next/font` in `app/layout.tsx`. `--font-heading` updated likewise.
- **Test added:** **TC-U-29** in Test Plan v1.3 — reads `getComputedStyle(...).fontFamily` on `<body>`, the detail-panel heading, and a tree row, asserting all three resolve to a font-family stack starting with `Inter` and not containing `Lora`. Closes the typeface-boundary verification trio with TC-U-02 (Summary Inter) and TC-U-03 (Prose Lora). Phase 3 active count moves from 98 to 99.
- **Re-test:** TC-U-02 / TC-U-03 / TC-U-29 all PASS. Visual confirmation: the running app now renders the tree, sidebar, header, and panel chrome in Inter, matching the wireframe.
...
- **Root cause:** `range.startContainer.parentElement.getBoundingClientRect()` returns the parent element's rect, not the cursor's line rect. For a wrapped or multi-line paragraph this is the wrong anchor.
- **Fix:** use `range.getBoundingClientRect()` directly. On a collapsed range (cursor only), this returns a 0-width rect at the **cursor's visual line** — exactly what the spec asks for. The parent element remains as a fallback for the degenerate case where the range rect has zero height (empty paragraph with a single `<br>`).
- **No spec change.** Brand Identity v2.0 §7.4 and Component Spec v2.4 §6.4 are correct as written; the implementation diverged.
- **Test impact:** TC-U-15 (typewriter scroll keeps active line near 42%) had a permissive `ratio < 0.60` upper bound that was wide enough to pass even with the parent-element bug. After the fix the cursor's line should land much closer to 0.42 — the test gets a stricter pass without any test-side change.
- **Re-test:** Focus Mode UI 8/8 + Editors UI 13/13 = 21/21 PASS.
...
| **SU-14 (resolved v1.6)** | Post-merge UX testing | **Structural UI rendered in browser default sans-serif, not Inter:** `app/globals.css` had a circular `--font-sans: var(--font-sans)` declaration that resolved to nothing, so non-editor surfaces inherited the browser default (Segoe UI / Arial / Helvetica Neue) instead of Inter. Brand Identity v2.0 §6 + Inviolable #4 both already required Inter; only the implementation diverged. **Resolved in v1.6** — `--font-sans: var(--font-inter)`. New TC-U-29 added (Test Plan v1.3) to guard the structural-Inter contract and close the verification trio with TC-U-02 / TC-U-03. |
...
**v1.6 — 2026-05-04** Structural-Inter corrective recorded. §3 gains a "Structural UI rendered in browser default, not Inter" entry classified as implementation gap. §4 SU-14 added and marked resolved. New TC-U-29 in Test Plan v1.3 closes the typeface-boundary verification trio (with TC-U-02 / TC-U-03). Active count moves 98 → 99; verdict 99/99 active PASS. No spec amendment — Brand Identity v2.0 §6 and Inviolable #4 were already correct; only the implementation diverged.
