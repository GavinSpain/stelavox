...
#### DR-036 — JSON import (round-trip restore from JSON backup)
- Origin: Phase 7 SHIPPED memo / CLAUDE.md v1.38
- Description: JSON export ships V1; the matching import (full backup restore) is V2.
- Effort: M  ·  V1-risk: Low
- Proposed bucket: V2+
- Rationale: Author has no realistic V1 need to import a backup (no other Stelavox instance to migrate from). Useful post-launch for support / migration scenarios.
...
#### DR-042 — SizeLimitWarning + EpubFallbackSuggestion dedicated components
- Origin: Phase 7 SHIPPED memo
- Description: Validator currently returns messages inline. Dedicated banner components for size-limit warnings + EPUB-fallback suggestions deferred.
- Effort: S  ·  V1-risk: None
- Bucket: **V1 — LOCKED 2026-06-10**, folded into a V1 polish pass (Phase 10), not standalone
- Rationale: Small and easy; author chose to land it inside an existing V1 pass. Verify message legibility in the modal while at it.
