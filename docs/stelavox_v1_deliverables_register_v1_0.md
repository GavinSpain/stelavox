...
- **Artifacts (approved 2026-06-14, build-ready):** `docs/wireframes/wireframe_export_format_lineup_v1.html` (wireframe approved) + `docs/stelavox_dr042_export_lineup_design_v1_0.md` v1.1 (Tier-B design note). Build-kickoff decisions **D1/D2/D3 all locked**: D1 picker trigger = node-count (>1 Book-layer node, not `document_type`); D2 = Markdown is its own tile; D3 = Series quick-export defaults to whole-series Markdown. No open decisions remain — ready to build at Phase 10 kickoff.
...
- **Capture cost:** Bands 1–4, 6, 7 + dependency status are **derive-only** (no migration). Bands 6/7 economics add **zero new capture** — `agent_jobs` already stores actual tokens + `cost_usd` + `model_id` + `route`; cadence from `stripe_price_id`. The only new capture (must-ship-before-Day-1, all small, no new tables): `export_jobs.file_size_bytes` column · realtime round-trip probe · queue-age in the minute rollup · conversation-window-pressure metric.
- Effort: M  ·  V1-risk: Medium (Day-1 capture window)
- Bucket: **V1 — LOCKED 2026-06-10; scope expanded + approved 2026-06-14.**
- **Artifacts (approved 2026-06-14, build-ready):** `docs/wireframes/wireframe_admin_ops_summary_v1.html` (Bands 1–7 approved) + `docs/stelavox_dr121_admin_ops_summary_design_v1_0.md` v1.1. Decisions **OA-1/2/3 locked**: bands atop `/admin`; new `/api/admin/ops` endpoint; realtime probe every 5 min. No open decisions remain.
