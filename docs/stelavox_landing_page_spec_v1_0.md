...
---
**Changelog**
**v1.5 — 2026-06-17** **Founding offer simplified — 50%/50% time-limited, partnership framing, pricing anchor.** Author traded the "50% off annual, for life" promise for **50% off your first year on annual, or 50% off your first month on monthly** — symmetric, simple, and entirely Stripe-native (no special code for perpetual discount tracking; the offer expires at its natural Stripe coupon duration). Same headline number on both cadences so messaging stays clean. Margins remain comfortable at 100% credit usage on the worst case (Writer Y1 founding annual at $8/mo eff. — $3.50 margin / 43.8%; Pro Y1 at $48/mo eff. — $24 margin / 50%); Year-2+ economics are actually *better* than lifetime-50% since the discount naturally expires at peak-LTV time. Added an **early-access partnership framing** on the founding band — "Beyond the discount, founding members get a direct line for feedback. We're launching in early access, and we want to build what comes next with serious writers — not just for them." Repositions the offer from "be first, get a discount" (transactional) to "founding partner shaping what comes next" (alignment with brand voice). Used **"early access"** not "beta" to avoid the buggy-software connotation for the non-technical writer audience. New FAQ "What does being a founding member mean?" — discount + early + voice in what we build next. New **price-anchor line** in the founding band — "Plans from $15/month — Writer to Pro, plus bring-your-own-key." — so the 50% discount has a concrete reference without committing to a full pricing-tier breakdown (Pro's exact price stays adjustable). Open-mode pricing teaser matched. Waitlist success message "founding offer" → "founding access" for terminology consistency. **Implementation note:** the offer requires only Stripe Dashboard admin (two coupons: `FOUNDING_ANNUAL_50` repeating 12 months + `FOUNDING_MONTHLY_30` once; the offer was bumped to 50% so the Monthly coupon will be `FOUNDING_MONTHLY_50` once) and a single config flip (`stripe.checkout.allow_promotion_codes = true`, already shipped key from Phase 9.B). Zero new code; founding members enter the promo code on Stripe's hosted Checkout. Closing the offer = disable the coupons in Stripe Dashboard; existing holders keep their discount until natural expiry. Spec §3.1 founding model superseded by this entry.
