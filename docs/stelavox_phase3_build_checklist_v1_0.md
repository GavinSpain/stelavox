...
- [ ] **T-6.5.** Create `components/focus/TypewriterContainer.tsx` per Component Spec §6.4. Keeps the active line at 42% of viewport height (±2px tolerance). `scroll-behavior: smooth`. On in Focus Mode by default; **the Edit-Mode opt-in via three-dot menu is deferred to Phase 8** (per Build Checklist v1.2 §6 SU-13 — the toggle host is bundled with Sentence Focus's Phase 8 work). Persists in `localStorage.stelavox_typewriter_enabled`. Acceptance: TC-U-15 passes.

- [ ] **T-6.6.** *(v1.2 amended — Phase-3 scope reduced to a CSS-only stub; full implementation deferred to Phase 8.)* Create `components/focus/SentenceFocus.tsx`. **Phase 3 stub only:** install the opacity CSS rules targeting `[data-sentence]` elements; default off; the toggle host (three-dot menu in prose editor panel header), the `Intl.Segmenter`-based segmentation, span wrapping, active/adjacent marking, and cursor-move transition wiring are all **Phase 8 work** per TA v1.7 §11 + Component Spec v2.5 §6.5. The stub satisfies the Phase 3 build's acceptance for "the file exists and exports the component"; the behavioural contract (Component Spec §6.5) lands in Phase 8. **Originally-scoped acceptance TC-U-14 / TC-M-04 / TC-M-06 are deferred to Phase 8** per Test Plan v1.2 §10.1.
