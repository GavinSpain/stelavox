...
**v1.0 — 2026-05-04 (Phase 4 close-out amendment row — SU-17)** `PATCH /api/nodes/[id]/move` now rejects context-node sources with `400 invalid_move_target`. Context nodes have `parent_id IS NULL`, no `layer_index`, and are not in the structural tree — moving one is meaningless. The amendment was added during Phase 4 implementation when context nodes were first creatable; the Phase 2 contract is otherwise unchanged. The new error code joins the §3.6 failure-mode list. Verified by Phase 4 Test Plan TC-A-32. This row is appended retroactively rather than bumping Phase 2 to v1.1 because Phase 2 itself shipped before context nodes existed, and re-versioning a closed-out contract would obscure the merge state at the time it was sealed. The amendment pattern matches the Phase 3 v1.1 PATCH `expected_version` retrofit precedent.
