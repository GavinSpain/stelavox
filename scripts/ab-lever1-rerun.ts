/**
 * Lever 1 rerun — re-synthesise prose for the existing Step-2 tighter beats
 * using the Migration-056 anti-bloat synthesise_beat prompt.
 *
 * Same Haiku model, same 7 tighter beats, only the synthesise prompt has
 * changed (three new anti-bloat disciplines).
 *
 * Usage:
 *   APP_URL=http://localhost:3000 npx tsx scripts/ab-lever1-rerun.ts
 */
...
const OUT_DIR = resolve(__dirname, '../docs/ab-lever1')
...
async function main() {
  ...
  console.log(`[lever1] synthesise_beat → ${HAIKU}`)
  ...
  // Reset Scene 1 beat prose only — keep the beat structure from Step 2.
  await db
    .from('nodes')
    .update({ prose: null })
    .eq('document_id', DOCUMENT_ID)
  ...
  console.log('[lever1] reset prose + conversations')
  ...
  try {
    ...
    const workflowId = await driveDirectorTurn(page, SYNTH_PROBE)
    console.log(`[lever1] synth workflow proposed: ${workflowId}`)
    ...
    while (Date.now() < deadline) {
      ...
      console.log(`[lever1] ${total - pending}/${total} done`)
    ...
    for (const s of synthSteps ?? []) {
      ...
      const summaryText = extractTiptapText(node?.summary)
    ...
    const out = {
      label: 'lever1-haiku-on-tighter-beats-with-anti-bloat',
      model_id: HAIKU,
      synth_workflow_id: workflowId,
    ...
    writeFileSync(resolve(OUT_DIR, 'lever1.json'), JSON.stringify(out, null, 2))
    console.log(`[lever1] saved → ${resolve(OUT_DIR, 'lever1.json')}`)
    console.log(
      `[lever1] cost=$${out.synth_cost_usd.toFixed(4)} ` +
