...
export default function DocumentMenu({ documentId, documentName, documentStatus }: Props) {
  ...
  async function handleRename(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(body.message ?? body.error ?? 'Failed to rename.')
      return
    }
    setRenameOpen(false)
    router.refresh()
  ...
  async function handleArchive() {
    setLoading(true)
    const newStatus = documentStatus === 'archived' ? 'active' : 'archived'
    const res = await fetch(`/api/documents/${documentId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border-default)' }}>
          ...
          <DropdownMenuItem onClick={handleArchive} style={menuItemStyle}>
      ...
      <Dialog open={renameOpen} onOpenChange={setRenameOpen}>
        <DialogContent style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border-default)' }}>
          <DialogHeader>
            <DialogTitle style={{ color: 'var(--color-text-primary)', fontSize: 'var(--text-lg)' }}>Rename document</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border-default)' }}>
          <DialogHeader>
            <DialogTitle style={{ color: 'var(--color-text-primary)', fontSize: 'var(--text-lg)' }}>Delete document?</DialogTitle>
          </DialogHeader>
          <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-secondary)', margin: 'var(--space-3) 0' }}>
            This will permanently delete <strong style={{ color: 'var(--color-text-primary)' }}>{documentName}</strong>. This cannot be undone.
          </p>
          <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end' }}>
