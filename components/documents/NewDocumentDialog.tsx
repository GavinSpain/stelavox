...
export default function NewDocumentDialog({ projectId }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/projects/${projectId}/documents`, {
      ...
      body: JSON.stringify({ name: name.trim(), description: description.trim() || null, document_type: docType }),
    })
    setLoading(false)
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      setError(body.message ?? body.error ?? 'Failed to create document.')
      return
    }
    setOpen(false)
    reset()
  ...
  return (
    <Dialog open={open} onOpenChange={o => { setOpen(o); if (!o) reset() }}>
      <DialogTrigger style={primaryButtonStyle}>New document</DialogTrigger>
