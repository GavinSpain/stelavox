...
function stage(
  ...
  // Test stages carry a prompt (the workflow-XOR-prompt rule from
  // M-183 requires one or the other). The cycle detector doesn't
  // care which.
  return {
    order,
    title: `Stage ${order}`,
    ...
    trigger_config,
    workflow: null,
    prompt: `Test stage ${order} prompt`,
  }
}

describe('detectStageTriggerCycles (V1.x-A.1, post-simplification)', () => {
  // 2026-05-21 simplification: trigger_type narrowed to ('after_stage',
  // 'manual'). 'compound' and 'scheduled_at' trigger types dropped.
  // Cycle detection now only considers after_stage edges.
  ...
  it('accepts a DAG with shared dependency', () => {
    const stages = [
      ...
      // Stage 4 depends on stage 3 (after_stage). The compound case
      // that previously combined multiple after_stage deps is no
      // longer expressible — stages chain linearly.
  ...
  it('treats manual stages as cycle-free', () => {
    const stages = [
      stage(1, 'manual'),
      stage(2, 'manual'),
    ]
