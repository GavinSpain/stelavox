-- Migration 089 — V1.x-A.1 follow-up: Director config v1.6.
-- Source: stelavox_director_architecture_v2_1_0.md §6 + post-V1.x-A.1
-- user-test discovery 2026-05-13.
--
-- v1.5 had two architectural issues exposed by the user-driven test:
--
-- 1. The model was instructed to call propose_brief AND echo the
--    validated proposal as a <brief_proposal> XML block in its text
--    content. The model reasonably treated the tool call as the
--    proposal and skipped the XML echo. Server parsed nothing.
--
-- 2. The v1.5 "Working pattern" section was too thin on chain-of-thought
--    scaffolding. Compare to Migration 062 (PLAN BEFORE YOU EXPAND) which
--    forces a structured planning pass before the JSON array. Without
--    that pattern, the Director would jump from read-tool-calls to
--    propose_brief without articulating the multi-stage shape — leading
--    to misfires on requests that needed dependency-aware stage planning.
--
-- v1.6 corrects both:
--
-- (a) Tool result IS the proposal. The system prompt no longer asks the
--     model to emit <brief_proposal> / <profile_amendment_proposal> XML.
--     The executor yields the brief_proposal / profile_amendment_proposal
--     events directly from the tool's validated return value (see
--     lib/director/executor.ts — accumulatedToolCalls.proposal_artefact).
--     Workflow proposals retain the XML mechanism (different architecture
--     that aggregates N step-tool results into one workflow shape).
--
-- (b) PLAN BEFORE YOU PROPOSE — a <plan>...</plan> scratchpad block,
--     modelled on Migration 062's PLAN BEFORE YOU EXPAND section. The
--     UI strips <plan> from rendered text (the executor's
--     consumeTextForDelta lists it in PROPOSAL_OPEN_TAGS). We persist
--     it in conversation_messages.content for debugging. The model
--     walks through a 5-6 step checklist before calling propose_brief
--     or propose_profile_amendment.
--
-- tool_suite unchanged from v1.5 (still 16 tools). Model config carried.

BEGIN;

UPDATE director_configs
SET status = 'deprecated', deprecated_at = NOW()
WHERE version_number = '1.5' AND status = 'production';
...
SELECT
  '1.6',
  'Director v1.6 — Plan-before-propose',
  'production',
  $PROMPT$# Director v1.6 system prompt
...
$PROMPT$,
  ...
  model_params,
  capability_flags,
  'V1.x-A.1 (v1.6) — Plan-before-propose + tool-call IS proposal. (1) System prompt rewritten with explicit <plan> scratchpad pattern modelled on Migration 062 (PLAN BEFORE YOU EXPAND): forces structural reasoning before calling propose_brief / propose_profile_amendment. UI strips <plan> from rendered text; persisted for debugging. (2) Drops the XML-echo requirement — model is told the tool call IS the proposal, no need to also emit <brief_proposal>/<profile_amendment_proposal> XML. The executor yields TurnEvents directly from the validated tool result. (3) Three-phase turn structure: Read → Plan in scratchpad → Commit. tool_suite unchanged from v1.5 (16 tools). Migration 089.'
FROM director_configs
WHERE version_number = '1.5';
