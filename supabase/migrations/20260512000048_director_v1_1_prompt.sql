-- Migration 048 — Director v1.1 system prompt revision.
--
-- V1.x-LB launch-blocker fix-pack, B2.
-- Source: docs/stelavox_director_architecture_v2_0.md §7.2 (range discipline),
--         project_v1x_lb_next_session_prep.md (B2 scope).
--
-- The 2026-05-10 launch test exposed three connected prompt-level
-- failures: (1) the Director did not know its own operational limits
-- (iteration cap, step cap, concurrent dispatch), so a 114-scene request
-- silently truncated mid-tool-use with turn_state=final and no
-- propose_workflow block; (2) batch operations had no canonical-range
-- discipline — workflow titles read coherently ("scenes 11-20") while
-- the underlying selection was scattered across the document; (3) no
-- defined protocol for handling requests that exceed the limits other
-- than silent truncation.
--
-- This migration ships Director v1.1 with four prompt additions:
--   - Brief stub  (end of "Tools you have")
--   - Your operational limits  (new section after "Working pattern")
--   - Canonical range discipline  (subsection inside "Workflow proposal")
--   - When a request exceeds your limits  (new section after "Workflow proposal")
--
-- Existing v1.0 text is preserved verbatim. Additions only.
--
-- Versioning. director_configs has UNIQUE(status) WHERE status='production',
-- so we must DEMOTE v1.0 first, then INSERT v1.1. Single transaction.
-- tool_suite / model_id / model_params / capability_flags are copied
-- from v1.0 untouched — only the system_prompt body changes here.
-- Extended thinking on Opus-class models lands in a later migration
-- in this same fix-pack (task 5).

BEGIN;

-- 1. Demote v1.0 to deprecated.
UPDATE director_configs
SET status = 'deprecated', deprecated_at = NOW()
WHERE version_number = '1.0' AND status = 'production';

-- 2. Insert v1.1 as the new production row, copying tool config from v1.0.
...
SELECT
  '1.1',
  'Director v1.1 — Production',
  'production',
  $PROMPT$
# Director v1.1 system prompt
...
$PROMPT$,
  ...
  model_params,
  capability_flags,
  'V1.x-LB B2 prompt revision. Adds: operational-limits surface (iteration/step/concurrent caps), canonical-range discipline for batch operations, multi-workflow batching protocol, Brief-tool forward stub. Migration 048. Director Architecture v2.0 §7.2.'
